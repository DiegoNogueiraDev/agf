...
>
> **Web local (`agf dashboard`):** SPA com 2 abas — **Grafo** (`@xyflow`, busca/filtros/drill-down) e **Economia** (custo real, economia delegate, cache local, levers). Mesma paleta do `agf` TUI/graph-flow.cloud. Sobe via `agf dashboard` ou `agf init` (serve automático).
