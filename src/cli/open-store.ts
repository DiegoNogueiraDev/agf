...
export function openStoreOrFail(dir: string, opts: OpenStoreOptions = {}): SqliteStore {
  if (opts.requireExisting === true) {
    // node_db03edaf7caa — em modo worktree-por-formiga os checks de existência/
    // anchor valem contra o root central do grafo, não contra o worktree.
    const root = resolveStoreRoot(dir)
    const dbPath = join(root, 'workflow-graph', 'graph.db')
    ...
    const hasAnchor = existsSync(join(root, 'package.json')) || existsSync(join(root, '.git'))
