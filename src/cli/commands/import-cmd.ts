...
export function importCommand(): Command {
  return new Command('import-prd')
    ...
    .option(
      '--strict-path',
      'Rejeita caminhos fora da raiz do projeto (path-traversal guard); requer .md/.txt/.html/.pdf/.prd',
      false,
    )
    .action(
      async (
        ...
        if (opts.strictPath) {
          try {
            filePath = (await readPrdFile(file)).absolutePath
          } catch (err) {
            out.err('PATH_TRAVERSAL', `Caminho rejeitado: ${getErrorMessage(err)}`)
            return
          }
        } else {
