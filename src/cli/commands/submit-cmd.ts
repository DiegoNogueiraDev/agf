...
import { buildConsumerProof, mergeConsumerProof, type ConsumerProof } from '../../core/certainty/consumer-proof.js'
 ...
 */
/**
 * Records consumer-proof into node.metadata additively (node_f7e5dbdbdf06).
 * Reads current metadata, merges the proof (immutable), persists via updateNode.
 * The timestamp is stamped here (Date.now) so the pure builder stays clockless.
 */
export function recordConsumerProof(
  store: SqliteStore,
  node: GraphNode,
  command: string,
  ...
  const proof = buildConsumerProof(command, result, Date.now(), evidence)
  const merged = mergeConsumerProof(node.metadata as Record<string, unknown> | undefined, proof)
  store.updateNode(node.id, { metadata: merged as GraphNode['metadata'] })
  return proof
}
...
export function submitCommand(): Command {
  ...
  cmd.option(
    '--consumer-proof <command>',
    'Registra prova de execução no modo do consumidor (o comando que rodou) em metadata.consumerProof',
  )
  cmd.option(
    '--consumer-proof-result <result>',
    'Resultado da prova do consumidor: passed|failed (default passed)',
    'passed',
  )
  cmd.option('--consumer-proof-evidence <text>', 'Evidência textual do que apareceu no modo do consumidor')
  cmd.action(
    (
      taskId: string,
      opts: {
        ...
        consumerProofResult?: string
        consumerProofEvidence?: string
      },
    ) => {
      ...
      // Standalone consumer-proof recording (node_f7e5dbdbdf06): `agf submit <id>
      // --consumer-proof "<cmd>"` with no executor result just fills
      // metadata.consumerProof so the Delivery Certainty pillar can go green.
      // Additive/immutable — never overwrites existing metadata (see mergeConsumerProof).
      if (opts.consumerProof && !opts.result && !opts.fromFile) {
        ...
        try {
          ...
          const result = opts.consumerProofResult === 'failed' ? 'failed' : 'passed'
          const proof = recordConsumerProof(store, node, opts.consumerProof, result, opts.consumerProofEvidence)
          out.ok({ taskId, consumerProof: proof, recorded: true })
          return
        } finally {
          store.close()
        }
