...
import { extractTaskFeatures, storeTaskFeatures } from '../../core/router/task-feature-extractor.js'
...
export function modelCommand(): Command {
  ...
  cmd
    .command('features')
    .description('Extrai e persiste o vetor de features RL (taskType/acCount/blastRadius/hasExternalDeps) de uma task')
    .argument('<id>', 'ID do node')
    .option('-d, --dir <dir>', 'Diretório do projeto', process.cwd())
    .action((id: string, opts: { dir: string }) => {
      const out = createCliOutput('model.features')
      const store = openStoreOrFail(opts.dir, { requireExisting: true })
      try {
        ...
        const features = extractTaskFeatures(node)
        storeTaskFeatures(store, id, features)
        out.ok({ id, features })
      } finally {
        store.close()
      }
