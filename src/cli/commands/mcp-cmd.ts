...
function mcpInspectCommand(): Command {
  return new Command('inspect')
    ...
    .action((opts: { name: string; url?: string; command?: string; args?: string; clientId?: string; scopes?: string }) => {
...
function mcpToolContextCommand(): Command {
  return new Command('tool-context')
    .description('Resolve whether MCP tool definitions load directly or defer to search, based on --threshold (no network calls)')
...
function mcpProbeCommand(): Command {
  return new Command('probe')
    .description('Attempt to connect to a configured MCP server and report reachability (makes a real network/spawn call)')
    .requiredOption('--name <name>', 'Server name')
