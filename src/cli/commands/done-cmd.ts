...
import { detectScopeCreep } from '../../core/gaps/detect-scope-creep.js'
...
function readHarnessScore(dir: string, memName: string): number | null {
...
function writeCompletionMemory(dir: string, id: string, title: string): string {
  const memDir = join(dir, STORE_DIR, 'memories')
  mkdirSync(memDir, { recursive: true })
  const name = `task-${id}`
  writeFileSync(join(memDir, `${name}.md`), `# ${title}\n\nTask \`${id}\` completed (DoD passed).\n`, 'utf-8')
  return name
}
    ...
    (
      ...
      try {
        ...
        if (!opts.force) {
          ...
          const undeclared = detectScopeCreep(modifiedFiles, declared)
        ...
        if (opts.agent) {
          const claimRelease = releaseTaskClaim(store.getDb(), taskId, opts.agent)
          if (claimRelease.mismatch) {
            const msg = `CLAIM_MISMATCH: task ${taskId} held by agent ${claimRelease.agentId ?? '?'}, not ${opts.agent}; proceeding anyway`
            log.warn(msg)
