...
import { surfaceProofState, surfaceProofBlocksDone } from '../../core/observability/scenario-verdict-store.js'
...
import { getCurrentSessionId } from '../../core/hooks/session-manifest.js'
  ...
  cmd.action(
    (
      ...
      try {
        ...
        // Surface-proof gate (node_4b365961b83a): a leaf that declared itself a
        // surface cannot be closed on an unproven claim. `missing` and
        // `inconclusive` block like `failed` — absence of proof is not proof.
        // Additive: a task with no scenario edge is never even consulted, so
        // non-surface behaviour is byte-identical. --force is the operator's
        // explicit consent, consistent with the other gates below.
        if (!opts.force) {
          const doc = store.toGraphDocument()
          const surfaceState = surfaceProofState(store.getDb(), taskId)
          if (surfaceProofBlocksDone(isSurfaceTask(doc, taskId), surfaceState)) {
            out.fail(
              'SURFACE_PROOF_MISSING',
              `Cannot mark "${taskId}" done: it declares itself a surface but its scenario proof is "${surfaceState}". Run the scenario for this node (only a concluded pass closes it), or use --force to override.`,
              { taskId, surface_proof: surfaceState },
            )
            return
          }
        }
        ...
        if (!opts.force) {
          ...
          if (!modified || modified.length === 0) {
            ...
            if (!committedDeliveryOnDisk) {
              ...
              // Antes de acusar "nada foi implementado", pergunte se existe
              // arquivo novo que o diff simplesmente não vê. Dizer isso a quem
              // ACABOU de criar o arquivo é o primeiro erro que um operador
              // novo encontra — e ele lê como "a ferramenta não funciona".
              const untracked = diagnoseUntracked(opts.dir)
              const message =
                untracked.length > 0
                  ? `No modified files found for "${taskId}" — but ${untracked.length} untracked file(s) exist, e.g.: ${untracked.slice(0, 5).join(', ')}. ` +
                    `The done gate reads git's index, so a new file is invisible until staged. For the file(s) YOU changed: ` +
                    `git add <file> && agf node update ${taskId} --implementation-files <file>, then retry.`
                  : `No modified files found for "${taskId}". Tasks must be implemented before marking done. Use --force to override. (git: ${gitError})`
              out.fail('NO_FILES_MODIFIED', message, { taskId, ...(untracked.length > 0 ? { untracked } : {}) })
              return
            }
        ...
        recordColonyOutcome(store, node, opts.dir)
        ...
        // Colony learning ticks (stagnation control + GA autotune) — extracted to
        // core/learning/post-done-ticks.ts: they teach the colony, they never decide
        // whether this task may close, and they never throw.
        const { stagnation } = runPostDoneLearning(store, taskId)
        ...
        recordDoneInManifest(taskId, opts.dir)

        recordSuccessLearning({ store, node, dir: opts.dir, grade: dod.grade, score: dod.score })
