...
export function contextCommand(): Command {
  ...
  const cmd = new Command('context')
    ...
    .action((id: string, opts: { compressed: boolean; full: boolean; format: string; dir: string }) => {
      ...
      try {
        // §node_cc4c4c7e02e2 — prefetch cache check: se a task solicitada
        // está no cache, serve sem re-computar (~0ms vs cold ~150ms).
        const cached = getPrefetchedContext(store, id)
        if (cached && opts.format === 'json') {
          out.ok({ cached: true, context: cached.context, brief: cached.brief, nodeId: id })
          return
        }
        if (cached && opts.format === 'markdown') {
          out.ok({ cached: true, markdown: cached.context, nodeId: id })
          return
        }
        ...
        if (flow) {
          ...
          out.ok({ ...flow.context, flow: toFlowEnvelopeBlock(flow) })
          return
        }
