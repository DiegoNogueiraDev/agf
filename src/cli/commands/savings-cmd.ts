...
export function savingsCommand(): Command {
  ...
  return new Command('savings')
    ...
    .option('--by-surface', 'Breakdown da economia por superficie do driver (hook/context/brief/envelope/internal)')
    .option(
      '--session [id]',
      'Recibo compacto de economia da sessão atual (ou de um id de sessão específico), por alavanca',
    )
    .action(
      (opts: {
        dir: string
        ...
        try {
          ...
          if (opts.session !== undefined) {
            // `--session <id>` pins that id; bare `--session` resolves the current sitting.
            const sessionId = resolveSessionId(store, {
              now: Date.now(),
              sessionId: typeof opts.session === 'string' ? opts.session : undefined,
            })
            out.ok(sessionSavingsReceipt(store.getDb(), sessionId))
            return
          }
          if (opts.bySurface) {
            out.ok(savingsBySurface(store.getDb()))
            return
          }
          if (opts.byCommand) {
