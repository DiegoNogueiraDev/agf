...
import { recordColonyBenchmark } from '../../core/swarm/colony-throughput-store.js'
...
export function swarmCommand(): Command {
  ...
  // ── Parallel batch pull (node_5e44fdf17849) ───────────────────────────────
  // Pulls the independent set into up-to-k atomic claims — WIP relaxed ONLY over
  // dependency-independent tasks; resource_locks + declared-file boundary keep two
  // ants off the same file/state. Report-only surface: it claims + returns the
  // batch (leases) for the operator/ants to run in their worktrees (delegate-first).
  dirOpt(
    cmd
      .command('batch')
      .description('Puxa o conjunto independente em até k claims paralelos (WIP relaxado só no set independente)')
      .option('-k, --size <n>', 'máximo de ants paralelos', '3'),
  ).action((opts: DirOpt & { size: string }) => {
    const out = createCliOutput('swarm.batch')
    const store = openStoreOrFail(opts.dir, { requireExisting: true })
    try {
      const k = Number(opts.size)
      const batch = pullIndependentBatch(store.toGraphDocument(), new LockManager(store.getDb()), k)
      out.ok({
        size: batch.length,
        claims: batch.map((c) => ({
          nodeId: c.node.id,
  ...
  // ── Serial vs. batch benchmark (node_wire_a54279a32eab) ───────────────────
  // Reuses pullIndependentBatch's own claim (via benchmarkColonyBatch) — proves
  // the overlap the batch pull promises instead of just asserting it. `run` has
  // no executor to inject here (delegate-first: core doesn't know providers), so
  // a synthetic delay stands in for per-task work, same as the module's own tests.
  dirOpt(
    cmd
      .command('bench')
      .description('Mede wall-clock serial vs. batch sobre o MESMO conjunto reivindicado (prova sobreposição real)')
      .option('-k, --size <n>', 'largura do batch', '4')
      .option('--delay-ms <n>', 'atraso sintético por task (ms) — substitui execução real', '40'),
  ).action(async (opts: DirOpt & { size: string; delayMs: string }) => {
    const out = createCliOutput('swarm.bench')
    const store = openStoreOrFail(opts.dir, { requireExisting: true })
    try {
      const delayMs = Number(opts.delayMs)
      const result = await benchmarkColonyBatch({
        doc: store.toGraphDocument(),
        makeLocks: () => new LockManager(store.getDb()),
        k: Number(opts.size),
        run: () => new Promise((resolve) => setTimeout(resolve, delayMs)),
      })
      // Grava o run: sem isto o ganho existe só no stdout desta execução e
      // `agf insights` não tem o que cobrar na sessão seguinte.
      recordColonyBenchmark(store.getDb(), store.getProject()?.id ?? '', result)
      out.ok(result)
    } finally {
      store.close()
    }
