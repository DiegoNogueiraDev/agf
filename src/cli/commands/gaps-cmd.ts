...
import { buildDriverBoundaryProbe } from '../../core/gaps/detect-driver-boundary.js'
...
import { makeCommitProbe } from '../shared/commit-probe.js'
      ...
      async (opts: {
        ...
        try {
          ...
          if (opts.apply) {
            ...
            const doc = store.toGraphDocument()
            const allGaps = detectAllGaps(doc, undefined, {
              fileExists: makeFileExists(opts.dir),
              driverBoundary: buildDriverBoundaryProbe(store, store.getDb()),
              // Código que entrou sem node (node_9bb2e60a6390). Janela curta de
              // propósito: histórico inteiro vira ruído e o sinal se perde.
              commitProbe: makeCommitProbe(opts.dir),
            })
          ...
          const rawGaps = detectAllGaps(doc, kinds, {
            fileExists: makeFileExists(opts.dir),
            driverBoundary: buildDriverBoundaryProbe(store, store.getDb()),
          })
          // O recorte por severidade acontece ANTES do relatório: pedir
          // `--severity required` e receber os recommended junto não é "dados a
          // mais", é uma afirmação falsa sobre o que é urgente (node_754040cd2680).
          const selected = filterGapsBySeverity(rawGaps, opts.severity as GapSeverity | undefined)
          const report = buildGapReport(sortGapsByImpact(selected, doc))
