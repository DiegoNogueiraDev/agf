...
import { analyzeLifecyclePipeline } from '../../core/planner/petri-net.js'
...
export function healCommand(): Command {
  ...
  return new Command('heal')
    ...
    .option(
      '--pipeline',
      'Valida deadlock/livelock do pipeline de 9 fases via Petri net (TB-CSPN), a partir da fase atual do grafo',
      false,
    )
    .action(
      (opts: {
        ...
        try {
          if (opts.pipeline) {
            ...
            const canonicalPhase = detectPhase({
              totalNodes: stats.totalNodes,
              backlog: stats.byStatus.backlog ?? 0,
              inProgress: stats.byStatus.in_progress ?? 0,
              done: stats.byStatus.done ?? 0,
            })
            const currentPhase = CANONICAL_TO_INTERNAL[canonicalPhase][0]
            const result = analyzeLifecyclePipeline(currentPhase)
            out.ok({
              pipeline: true,
              canonicalPhase,
              currentPhase,
              deadlock: result.deadlock,
              reachablePlaces: result.reachablePlaces,
              hint: result.deadlock
                ? `Deadlock detectado a partir de ${currentPhase} — pipeline travada.`
                : `Pipeline saudável — sem deadlock a partir de ${currentPhase}.`,
            })
            return
          }
