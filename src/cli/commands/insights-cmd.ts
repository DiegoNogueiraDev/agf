...
export function insightsCommand(): Command {
  ...
  dirOpt(cmd.command('summary', { isDefault: true }).description('Resumo de fluxo')).action((opts: { dir: string }) =>
    withStore(opts, (store) => {
      ...
      // Flow efficiency (state-based snapshot proxy, 0–100): share of pulled work
      // actively moving (in_progress) vs waiting (ready + blocked). A deterministic
      // stand-in for touch-time/lead-time when per-state durations aren't tracked.
      const bs = store.getStats().byStatus
      const active = bs.in_progress ?? 0
      const waiting = (bs.ready ?? 0) + (bs.blocked ?? 0)
      const flowEfficiency = active + waiting > 0 ? Math.round((active / (active + waiting)) * 100) : 0
      out.ok({ ...metrics, wip, bottlenecks, flowEfficiency })
    }),
  )
