...
import { summarizeThroughput } from '../../core/swarm/colony-throughput-store.js'
...
export function insightsCommand(): Command {
  ...
  dirOpt(cmd.command('summary', { isDefault: true }).description('Resumo de fluxo')).action((opts: { dir: string }) =>
    withStore(opts, (store) => {
      ...
      // Throughput MEDIDO do run paralelo (node_414bf84a63d8). Não é derivado das
      // contagens do grafo: vem do wall-clock que `agf swarm bench` gravou. Sem
      // run gravado, sai `no-data` — a mesma regra do cockpit de OKR, porque um
      // speedup afirmado sem medição vale menos que nenhum número.
      const parallelThroughput = summarizeThroughput(store.getDb(), store.getProject()?.id ?? '')
      out.ok({
        ...metrics,
        wip,
        bottlenecks,
        flowEfficiency: velocityScorecard.flowEfficiency,
        velocityScorecard,
        parallelThroughput,
      })
    }),
