...
import { renewTaskClaim, activeTaskLeaseOwner } from '../../core/planner/renew-task-claim.js'
import { claimedByOf } from '../../core/planner/next-task.js'
  ...
  cmd
    ...
    .action((id: string, state: string, opts: { dir: string; force: boolean }) => {
      ...
      withStore(opts.dir, (store) => {
        ...
        // node_c17f488f93e3 — pull direcionado registra dono: com identidade,
        // in_progress grava claimedBy (o marcador que anti-hijack, fronteira de
        // arquivos e claims --colony leem). Dono existente DIFERENTE ⇒ recusa
        // sem --force (a task é de outra formiga); sem identidade ⇒ byte-idêntico.
        const callerId = process.env.AGF_AGENT_ID
        if (to === 'in_progress' && callerId) {
          const owner = claimedByOf(node)
          if (owner !== undefined && owner !== callerId && !opts.force) {
            // node_0c28154d4517 — dono estrangeiro só vale se a task ESTÁ em
            // voo (in_progress = feromônio durável) ou se a lease do pull está
            // viva (janela de atomicidade). Dono órfão (backlog/ready + lease
            // morta) é assumível: a formiga original nunca voou.
            const inFlight = node.status === 'in_progress'
            const leaseOwner = activeTaskLeaseOwner(store.getDb(), id)
            if (inFlight || leaseOwner === owner) {
              out.err(
                'FOREIGN_WIP',
                `Nó "${id}" está em voo de outra formiga (claimedBy=${owner}). Use --force apenas se tiver certeza de que a dona morreu.`,
              )
              return
            }
          }
        ...
        if (to === 'in_progress' && callerId) {
          store.updateNode(id, { metadata: { ...(node.metadata ?? {}), claimedBy: callerId } })
        }
        // node_728743c96bd9 — heartbeat da colônia: marcar in_progress com
        // identidade renova a lease do dono (sem identidade/mismatch ⇒ no-op).
        if (to === 'in_progress') {
          renewTaskClaim(store.getDb(), id, process.env.AGF_AGENT_ID)
        }
