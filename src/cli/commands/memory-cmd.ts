...
import {
  ...
  readAllMemories,
  amortizeFromLevers,
  type MemoryAmortizeOptions,
} from '../../core/memory/memory-reader.js'
import { buildMemoryPack, DEFAULT_PACK_BUDGET_TOKENS } from '../../core/memory/memory-pack.js'
import { resolveEconomyLeversConfig } from '../../core/economy/economy-levers-config.js'
...
import { createHonchoProviderFromEnv } from '../../core/memory/honcho-provider.js'
...
/**
 * Lê as levers do projeto (best-effort) e mapeia p/ opções de amortize-on-write.
 * Sem store/config ⇒ undefined ⇒ writeMemory segue byte-idêntico (default-OFF).
 */
function readAmortizeLevers(dir: string): MemoryAmortizeOptions | undefined {
  const store = openStoreIfExists(dir)
  if (!store) return undefined
  try {
    return amortizeFromLevers(resolveEconomyLeversConfig(store))
  } catch {
    return undefined
  } finally {
    store.close()
...
export function memoryCommand(): Command {
  ...
  cmd
    ...
    .action(async (name: string, opts: { content?: string; file?: string; dir: string }) => {
      ...
      try {
        // node_5c8bbec46123 — amortizar na escrita: levers ncd_dedup/memory_salience
        // (default-OFF ⇒ amortize undefined ⇒ caminho byte-idêntico ao atual).
        const result = await writeMemory(opts.dir, name, content, { amortize: readAmortizeLevers(opts.dir) })
        out.ok({
          name,
          bytes: Buffer.byteLength(content),
          ...(result.outcome === 'fused' ? { fused: true, fusedInto: result.fusedInto } : {}),
          citations: groundCitations(content),
        })
      } catch (e) {
  ...
  cmd
    .command('prefetch')
    .description(
      'Busca memórias via MemoryManager (builtin + honcho opcional) e retorna um bloco fenced pronto para prompt',
    )
    .requiredOption('--session-id <id>', 'ID da sessão (usado no ConversationContext)')
    .option('-d, --dir <dir>', 'Diretório do projeto', process.cwd())
    .action(async (opts: { sessionId: string; dir: string }) => {
      const out = createCliOutput('memory.prefetch')
      const external = createHonchoProviderFromEnv(process.env)
      const manager = new MemoryManager(new BuiltinMemoryProvider(opts.dir), external)
      const results = await manager.prefetchAll({ sessionId: opts.sessionId, recentMessages: [] })
      out.ok({ results, fencedBlock: manager.buildFencedBlock(results) }, { count: results.length })
    })

  cmd
    .command('pack')
    .description('Monta o pack de memória top-N por salience sob um teto de tokens (node_2a8c83993c72)')
    .option('-d, --dir <dir>', 'Diretório do projeto', process.cwd())
    .option('--budget <n>', 'Teto de tokens do pack', String(DEFAULT_PACK_BUDGET_TOKENS))
    .action(async (opts: { dir: string; budget: string }) => {
      const out = createCliOutput('memory.pack')
      const budgetTokens = parseInt(opts.budget, 10) || DEFAULT_PACK_BUDGET_TOKENS
      const store = openStoreIfExists(opts.dir)
      try {
        const ledger = store ? { db: store.getDb() } : undefined
        const pack = await buildMemoryPack(opts.dir, { budgetTokens, ledger })
        out.ok(pack ?? { entries: [], tokens: 0, totalCandidates: 0 })
      } catch (e) {
        out.err('PACK_FAILED', `Falha ao montar pack de memória: ${errMessage(e)}`)
      } finally {
        store?.close()
      }
    })
