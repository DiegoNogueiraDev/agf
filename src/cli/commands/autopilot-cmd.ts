...
import { runAutopilot, type ImplementResult } from '../../core/autonomy/autopilot-loop.js'
...
import { maybeDelegateToSwarm } from '../shared/swarm-delegation.js'
...
export function autopilotCommand(): Command {
  ...
  return new Command('autopilot')
    ...
    .option(
      '--swarm',
      'Se o binário ant-swarming estiver instalado, delega a orquestração a ele (`ant-swarming run`); ausente → fluxo atual',
      false,
    )
    .option('--ants <n>', 'Nº de formigas ao delegar via --swarm', '2')
    .action(
      async (opts: {
        dir: string
        max: string
        ...
        try {
          // Opt-in --swarm: se o binário ant-swarming está instalado, delega a
          // orquestração a ele e repassa o envelope. Ausente ⇒ NÃO toca o fluxo
          // atual (cai fora do if e segue o caminho delegado/live de sempre).
          if (opts.swarm) {
            const swarm = await maybeDelegateToSwarm({ dir: opts.dir, ants: parseInt(opts.ants, 10) || 2 }, {})
            if (swarm.delegated) {
              out.ok({ mode: 'swarm-delegated', swarm: swarm.status, run: swarm.envelope })
              store.close()
              return
            }
            progress('[SWARM] ant-swarming não instalado — seguindo o fluxo delegado/live atual.\n')
          }
          ...
          let implement: (node: {
            id: string
            title: string
          }) => boolean | ImplementResult | Promise<boolean | ImplementResult>
