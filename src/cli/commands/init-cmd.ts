...
import { maybeScaffoldStarter } from '../../core/init/scaffold-guided-starter.js'
...
export function initCommand(): Command {
  return new Command('init')
    ...
    .option('--guided', 'Declara a intenção de scaffold inicial (um grafo vazio já recebe o starter por padrão)', false)
    .option(
      '--graph-only',
      ...
      async (opts: {
        ...
        try {
          ...
          if (!success) {
            ...
            // Sempre tenta: num grafo vazio o operador novo precisa de algo
            // pullable, e o scaffold recusa sozinho qualquer grafo com trabalho.
            {
              const store = openStoreOrFail(dir, { requireExisting: true })
              try {
                guided = maybeScaffoldStarter(store, { guidedFlag: opts.guided })
              } finally {
                store.close()
              }
            }
