    ...
    .action(
      async (opts: {
        ...
        if (opts.gate && opts.suite === 'economy') {
          ...
          let scenarios
          try {
            // strict: a malformed scenario.json fails the gate loudly, naming the file (AC3).
            scenarios = loadSuite(economyDir, opts.tier, { strict: true })
          } catch (err) {
            out.err('MALFORMED_SCENARIO', String(err instanceof Error ? err.message : err))
            return
          }
