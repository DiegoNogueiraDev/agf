...
export function buildBoard(store: DocStore, swimlaneMode: SwimlaneMode = 'none'): KanbanBoard {
  return buildKanbanBoard(store.toGraphDocument(), { ...DEFAULT_KANBAN_CONFIG, swimlaneMode })
}
...
export function kanbanCommand(): Command {
  ...
  const cmd = new Command('kanban')
    ...
    .action((opts: { dir: string; swimlane: string; suggestions: boolean }) => {
      ...
      try {
        ...
        const board = buildBoard(store, mode)
