...
/** Status do 2º binário `ant-swarming`: instalado (colônia) ou ausente (modo delegado atual). */
export interface SwarmingCliStatus {
  installed: boolean
  version?: string
  capabilities?: string[]
}

/** Timeout curto do handshake — qualquer travamento acima disso ⇒ {installed:false}. */
const SWARMING_HANDSHAKE_TIMEOUT_MS = 1500

/**
 * Detecta se o orquestrador `ant-swarming` está instalado, executando o
 * subcomando `handshake` e validando o contrato por Zod (fonte única em
 * schemas/swarming-handshake). PORQUÊ: o agf decide colônia vs. modo delegado —
 * na AUSÊNCIA, NADA muda (delegate-first é regra do repo). Nunca propaga throw:
 * binário ausente, timeout, stdout inválido ou fora-do-contrato ⇒ {installed:false}.
 */
export async function detectSwarmingCli(opts?: {
  bin?: string
  env?: NodeJS.ProcessEnv
  timeoutMs?: number
}): Promise<SwarmingCliStatus> {
  const bin = opts?.bin ?? 'ant-swarming'
  const timeout = opts?.timeoutMs ?? SWARMING_HANDSHAKE_TIMEOUT_MS
  return new Promise((resolve) => {
    execFile(bin, ['handshake'], { timeout, env: opts?.env ?? process.env }, (err, stdout) => {
      if (err) return resolve({ installed: false })
      try {
        const envelope = JSON.parse(stdout) as { data?: unknown }
        const parsed = swarmingHandshakeSchema.safeParse(envelope.data)
        if (!parsed.success) return resolve({ installed: false })
        resolve({ installed: true, version: parsed.data.version, capabilities: parsed.data.capabilities })
      } catch {
