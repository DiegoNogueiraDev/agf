...
import { type ImplementResult } from '../../core/autonomy/autopilot-loop.js'
...
export interface LiveImplement {
  ...
  implement: (node: { id: string; title: string }) => Promise<ImplementResult>
...
export function buildLiveImplement(options: LiveImplementOptions): LiveImplement {
  ...
  const implement = async (node: { id: string; title: string }): Promise<ImplementResult> => {
    ...
    // node_a540ef426973 — devolve o ImplementResult (não só o boolean) para o
    // autopilot capturar o motivo REAL da falha (ex.: 401 do provider) em
    // escalationContext.lastError, em vez de escalar com motivo vazio.
