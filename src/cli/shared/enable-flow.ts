...
/**
 * Liga/desliga o experimento A/B (`experiment.abEnabled`), preservando o resto
 * do flow_config — sem A/B todas as chamadas caem no braço flow_on e o
 * flow-report nunca consegue comparar (veredito fica inconclusive).
 */
export function setFlowAbEnabled(store: FlowToggleStore, abEnabled: boolean): void {
  const raw = store.getProjectSetting(FLOW_CONFIG_SETTING_KEY)
  let current: Record<string, unknown> = {}
  if (raw) {
    try {
  ...
  const experiment = { ...(current.experiment as Record<string, unknown> | undefined), abEnabled }
  store.setProjectSetting(FLOW_CONFIG_SETTING_KEY, JSON.stringify({ ...current, experiment }))
}
