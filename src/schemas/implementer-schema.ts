...
export const DodCheckSchema = z.object({
  ...
  /** Quick-fix suggestion for failed checks. Always present on failed checks. */
...
export const ImplementDoneReportSchema = z.object({
  ...
  summary: z.string(),
  /**
   * True quando a task se declara surface via aresta (related_to|implements) a
   * um nó scenario|browser_test (node_919687afcea8). Opcional/backward-compat:
   * ausente ou false ⇒ não-surface, gate byte-idêntico ao de hoje.
   */
