...
function publishStep(): Step {
  // The publish step lives in the self-hosted `release` job, not the first (`validate`) job —
  // search every job's steps, not just jobs[0].
...
describe('package-bun publish gate', () => {
  ...
  it('the gate publishes only on a release commit or a manual dispatch', () => {
    // The step runs only when the `PUBLISHING` env is true, and PUBLISHING is set from the
    // release event or a manual workflow_dispatch — so the release gate lives on that env.
    expect(publishStep().if ?? '').toMatch(/PUBLISHING/)
    const raw = readFileSync(WF, 'utf8')
    expect(raw).toMatch(/event_name == 'release'|workflow_dispatch/)
  })
})
