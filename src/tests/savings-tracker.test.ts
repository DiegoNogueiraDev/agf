...
describe('AC3: delegate mode tokens_saved=0 → delegateSaved from cmdTok vs baselineTok', () => {
  ...
  it('multiple commands accumulate correctly', () => {
    const store = freshStore()
    // 2 commands, each with 100 cmdTok and graphExportBytes=2000
    // Total: cmdTok=200, baselineTok=1000, delegateSaved=800
    insertCmd(store, { inputBytes: 200, outputBytes: 200, graphExportBytes: 2000 })
    insertCmd(store, { inputBytes: 200, outputBytes: 200, graphExportBytes: 2000 })
    ...
    expect(report.delegateEconomy!.baselineTok).toBe(1000)
    expect(report.delegateEconomy!.delegateSaved).toBe(800)
    store.close()
  })
})
