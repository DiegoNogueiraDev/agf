...
import {
  ...
  recordModelCall,
  summarizeLedger,
  summarizeConductorCost,
} from '../core/observability/llm-call-ledger.js'
...
describe('summarizeConductorCost — self-reported cost, separate from agf own (node_a6314d7eef0a)', () => {
  it('AC2: fail-safe — no rows returns all zeros with the self_reported label', () => {
    const db = freshDb()
    const c = summarizeConductorCost(db)
    expect(c.source).toBe('self_reported')
    expect(c.calls).toBe(0)
    expect(c.tokensIn).toBe(0)
    expect(c.tokensOut).toBe(0)
    expect(c.byNode).toEqual([])
    db.close()
  })

  it('sums ONLY provider=delegated rows (conductor), excluding agf own calls, keyed by task/session', () => {
    const db = freshDb()
    // Conductor self-reported rows (provider='delegated').
    recordModelCall(db, {
      sessionId: 'sess_a',
      nodeId: 'task_1',
      provider: 'delegated',
    ...
    // agf's own call (a real gateway provider) — must NOT count toward consumer cost.
    recordModelCall(db, {
      sessionId: 'sess_a',
      nodeId: 'task_1',
      provider: 'anthropic',
    ...
    const c = summarizeConductorCost(db)
    expect(c.calls).toBe(2) // only the 2 delegated rows
    expect(c.tokensIn).toBe(400)
    expect(c.tokensOut).toBe(160)
    expect(c.byNode).toEqual([{ nodeId: 'task_1', tokensIn: 400, tokensOut: 160 }])
    expect(c.bySession[0]?.sessionId).toBe('sess_a')
    db.close()
  })
})
