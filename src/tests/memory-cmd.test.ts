...
describe('memoryCommand', () => {
  ...
  it('wires 9 subcommands', () => {
    expect(memoryCommand().commands.length).toBe(9)
  })
})
...
describe('memoryCommand pack (node_wire_6f361620ae9a — memory-pack wire)', () => {
  let dir: string
  ...
  async function runPack(args: string[]): Promise<Record<string, unknown>> {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    ...
    try {
      await memoryCommand().parseAsync(['pack', ...args], { from: 'user' })
    } finally {
      spy.mockRestore()
    }
    return JSON.parse(out.join('').trim())
  ...
  it('AC1: budget caps the pack — top-N by salience under the token ceiling', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-memory-pack-'))
    const phrase = 'lição durável do ciclo sobre gates, leases e worktrees da colônia. '
    for (let i = 0; i < 24; i += 1) {
      await writeMemory(dir, `mem-${String(i).padStart(2, '0')}`, `entrada ${i}: ${phrase.repeat(2)}`)
    }

    const envelope = await runPack(['-d', dir, '--budget', '200'])

    expect(envelope.ok).toBe(true)
    const data = envelope.data as { entries: unknown[]; tokens: number; totalCandidates: number }
    expect(data.tokens).toBeLessThanOrEqual(200)
    expect(data.entries.length).toBeGreaterThan(0)
    expect(data.totalCandidates).toBe(24)
  })

  it('AC2: no memories yet ⇒ empty pack, no error', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-memory-pack-empty-'))
    const envelope = await runPack(['-d', dir])

    expect(envelope.ok).toBe(true)
    const data = envelope.data as { entries: unknown[]; tokens: number }
