...
// ── Goodhart guard: how strongly was a pass corroborated? (node_491cd48b6a54) ──
//
// The identity and effect checks are opt-in by design (backward compat). That
// leaves the risk this node names structurally alive: a scenario whose steps are
// trivial — never clicking the real control — passes simply by omitting both
// fields. The oracle cannot refuse it without breaking every existing scenario,
// so instead it must SAY how weak the pass was, letting the KR count trivial
// greens rather than treating them as equal evidence.
describe('pass corroboration (Goodhart guard)', () => {
  const terminal = (over: Partial<StepResult> = {}): StepResult => ({
    tool: 'click',
    ok: true,
    evidence: 'shot.png',
    concludes: true,
  ...
  it('marks a bare pass as uncorroborated — evidence alone proves arrival, not effect', () => {
    const v = evaluateScenario([terminal()])
    expect(v.verdict).toBe('passed')
    expect(v.corroboration).toBe('none')
  })

  it('counts a checked identity as corroboration', () => {
    const v = evaluateScenario([terminal({ expectedIdentity: '/x', observedIdentity: '/x' })])
    expect(v.verdict).toBe('passed')
    expect(v.corroboration).toBe('identity')
  })

  it('counts a proven effect as corroboration', () => {
    const v = evaluateScenario([terminal({ expectsEffect: true, crossCheck: [{ before: 1, after: 2 }] })])
    expect(v.verdict).toBe('passed')
    expect(v.corroboration).toBe('effect')
  })

  it('reports both when the scenario proves arrival AND effect', () => {
    const v = evaluateScenario([
      terminal({
        expectedIdentity: '/x',
        observedIdentity: '/x',
        expectsEffect: true,
        crossCheck: [{ before: 1, after: 2 }],
      }),
    ])
    expect(v.corroboration).toBe('both')
  })

  it('does not claim corroboration for a verdict that never passed', () => {
    // A failed or inconclusive run has nothing to corroborate; reporting a level
    // there would invite counting it as partial evidence.
    expect(evaluateScenario([]).corroboration).toBeUndefined()
    expect(evaluateScenario([terminal({ ok: false })]).corroboration).toBeUndefined()
  })
})
