...
describe('coluna surface — atribuicao por superficie do driver (F2.T2 node_41efe88b1dcd)', () => {
  it('recordLeverEvent grava surface e a leitura retorna o valor', () => {
    const id = recordLeverEvent(db, {
      sessionId: 's1',
      lever: 'exec_compress',
      tokensBefore: 100,
      tokensAfter: 60,
    ...
    const row = db.prepare('SELECT surface FROM economy_lever_ledger WHERE id = ?').get(id) as { surface: string }
    expect(row.surface).toBe('hook')
  })

  it('linha pre-migracao (surface NULL) le sem erro e o summarize continua funcionando', () => {
    db.prepare(
      `INSERT INTO economy_lever_ledger
       ...
       VALUES ('legacy1', 1, 's0', NULL, 'heat_kernel', 10, 5, 5, 1, 'accepted', NULL, NULL)`,
    ).run()
    const row = db.prepare('SELECT surface FROM economy_lever_ledger WHERE id = ?').get('legacy1') as {
      surface: string | null
    }
    expect(row.surface).toBeNull()
    expect(summarizeByLever(db).length).toBeGreaterThanOrEqual(1)
  })
})
