...
// node_616e64c1a5ad — novo colaborador do --economy-report. Segue a convenção
// deste arquivo (FAKE_DB é {} e todo colaborador que toca o banco é stubado);
// o comportamento REAL de buildCascadeAbDelta é provado em
// cascade-ab-delta.test.ts contra um SQLite :memory: de verdade.
vi.mock('../core/economy/cascade-ab-delta.js', () => ({
  buildCascadeAbDelta: vi.fn(() => ({
    hasData: false,
    taskCount: 0,
    tokensBefore: 0,
    tokensAfter: 0,
...
// collectVelocityScorecard (node_d35e86e659dc) percorre grafo+ledger; no
// harness de envelope estes stubs degradam a velocity a zeros+note sem throw.
const VELOCITY_STORE_STUBS = {
  getProject: () => null,
  toGraphDocument: () => ({ version: 1, nodes: [], edges: [] }),
  getStats: () => ({ total: 0, byStatus: {}, byType: {} }),
}
