...
// Regression (node_f0a947d02eb7): filesystem sandbox escape. entryMatchesPath used
// startsWith without a path boundary and resolveCandidatePath did not normalize '..'
// on absolute paths, so a Tmpdir/Path sandbox granted access outside its root.
describe('resolveAccess — path boundary + traversal (sandbox escape regression)', () => {
  const cwd = '/home/user/project'
  const tmpdirPolicy: FileSystemSandboxPolicy = {
    kind: 'Restricted',
    entries: [{ path: { type: 'Special', value: 'Tmpdir' }, access: 'Write' }],
  }
  const projPolicy: FileSystemSandboxPolicy = {
    kind: 'Restricted',
    entries: [{ path: { type: 'Path', path: '/home/user/proj' }, access: 'Write' }],
  }

  it('still grants Write inside the real /tmp root', () => {
    expect(resolveAccess('/tmp/safe.txt', cwd, tmpdirPolicy)).toBe('Write')
  })

  it('DENIES a sibling directory that only shares the /tmp prefix (/tmpevil)', () => {
    expect(resolveAccess('/tmpevil/secret', cwd, tmpdirPolicy)).toBe('Deny')
  })

  it('DENIES a traversal escape out of /tmp via ..', () => {
    expect(resolveAccess('/tmp/../etc/passwd', cwd, tmpdirPolicy)).toBe('Deny')
    expect(resolveAccess('/tmp/../../root/.ssh/id_rsa', cwd, tmpdirPolicy)).toBe('Deny')
  })

  it('DENIES a Path-type sibling that only shares the prefix (proj vs proj-secrets)', () => {
    expect(resolveAccess('/home/user/proj/a.ts', cwd, projPolicy)).toBe('Write')
    expect(resolveAccess('/home/user/proj-secrets/leak', cwd, projPolicy)).toBe('Deny')
  })
})
