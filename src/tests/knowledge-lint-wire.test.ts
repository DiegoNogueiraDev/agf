...
describe('lintKnowledge extended findings', () => {
  ...
  it('emits under_linked_entity for a term in ≥3 facts with zero edges (AC4, node_wire_5d790ec275f0)', () => {
    const db = makeDb()
    db.prepare('INSERT INTO memory_facts (id, content) VALUES (?, ?)').run('f1', 'authentication token jwt')
    db.prepare('INSERT INTO memory_facts (id, content) VALUES (?, ?)').run('f2', 'authentication session cookie')
    db.prepare('INSERT INTO memory_facts (id, content) VALUES (?, ?)').run('f3', 'authentication oauth bearer')
    const result = lintKnowledge(db)
    const underLinked = result.findings.filter((f) => f.reason === 'under_linked_entity')
    expect(underLinked.some((f) => f.id === 'authentication')).toBe(true)
  })

  it('does not emit under_linked_entity once the term has an edge (AC4)', () => {
    const db = makeDb()
    db.prepare('INSERT INTO memory_facts (id, content) VALUES (?, ?)').run('f1', 'authentication token jwt')
    db.prepare('INSERT INTO memory_facts (id, content) VALUES (?, ?)').run('f2', 'authentication session cookie')
    db.prepare('INSERT INTO memory_facts (id, content) VALUES (?, ?)').run('f3', 'authentication oauth bearer')
    ...
    const result = lintKnowledge(db)
    const underLinked = result.findings.filter((f) => f.reason === 'under_linked_entity')
    expect(underLinked.some((f) => f.id === 'authentication')).toBe(false)
  })
})
