...
describe('computeOkrStatus — deadline vencida sem âncora de início', () => {
  // Um prazo vencido com o alvo não atingido é at-risk INDEPENDENTE de quando a
  // janela começou: não é preciso saber o início para saber que o fim passou.
  // Exigir a âncora antes dessa checagem transformava épicos sem `createdAt`
  // (existem no grafo real) em `no-data` permanente — e com eles o filtro
  // `--at-risk` ficava sobre um domínio que nunca se povoava.
  it('an overdue deadline is at-risk even with no start anchor', () => {
    const verdict = computeOkrStatus({
      kr: { target: 100, current: 40, unit: null, attainment: 0.4, status: 'ok', provenance: 'metadata' },
  ...
  it('a deadline still ahead with no start anchor stays no-data — pace is genuinely unjudgeable', () => {
    const verdict = computeOkrStatus({
      kr: { target: 100, current: 40, unit: null, attainment: 0.4, status: 'ok', provenance: 'metadata' },
    ...
    // Sem início não há régua; e sem régua o honesto é dizer que não se sabe —
    // jamais chutar on-track.
