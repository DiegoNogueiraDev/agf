...
// ── Declared scope is read from where node update WRITES it (node_93d60a427277) ──
//
// surgical_scope_pass read `metadata.declaredFiles`, a path nothing in the CLI
// ever writes: `node update --implementation-files` sets the top-level
// implementationFiles/testFiles. So the check skipped on EVERY node and its
// "no declared scope" reason was false. A skip that counts as green is worse
// than a failure — a node with scope and one without score identically, and the
// blast-radius gate depends on exactly this field.
describe('surgical_scope_pass — reads the declared scope that actually exists', () => {
  const scoped = (overrides: Partial<GraphNode> = {}): GraphNode =>
    makeNode({
      status: 'in_progress',
      acceptanceCriteria: ['Dado X, quando Y, entao Z'],
      implementationFiles: ['src/core/a.ts'],
      testFiles: ['src/tests/a.test.ts'],
      ...overrides,
    })

  const scopeCheck = (node: GraphNode) =>
    checkDefinitionOfDone(makeDoc([node]), node.id).checks.find((c) => c.name === 'surgical_scope_pass')

  it('does NOT report "no declared scope" for a node carrying implementationFiles', () => {
    const check = scopeCheck(scoped())
    expect(check).toBeDefined()
    expect(check?.details).not.toContain('No declared scope')
  })

  it('complexity budget analyses the files the task declares, not an empty touched set', () => {
    // Its input was `touched`, which is only populated at done-time — so at check
    // time it always reported "no implementation files declared" even for a node
    // that declares them. The message described a state that was not the reason.
    const check = checkDefinitionOfDone(makeDoc([scoped()]), 'node_test').checks.find(
      (c) => c.name === 'complexity_budget_pass',
    )
    expect(check?.details).not.toContain('Sem arquivos de implementação declarados')
  })

  it('still skips for a node that genuinely declares nothing', () => {
    // The legitimate skip must survive: the fix must not invert it into a failure.
    const check = scopeCheck(scoped({ implementationFiles: undefined, testFiles: undefined }))
    expect(check?.details).toContain('No declared scope')
  })
})
