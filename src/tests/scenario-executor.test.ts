...
import { JourneyRunVerdictSchema } from '../schemas/journey-run.schema.js'
import { HarnessRunVerdictSchema } from '../schemas/browser-harness.schema.js'
...
const PLAN_3STEP: ScenarioPlan = {
  steps: [
    {
      raw: 'go to site',
      tool: 'browser_navigate',
      args: { url: 'http://ex.com/import' },
      confidence: 1,
      needsDelegation: false,
    },
    { raw: 'click submit', tool: 'browser_click', args: { x: 1, y: 2 }, confidence: 1, needsDelegation: false },
    { raw: 'confirm', tool: 'browser_click', args: { x: 3, y: 4 }, confidence: 1, needsDelegation: false },
  ],
  unresolved: 0,
}

describe('executeScenario — L8 (node_902b00d0749d): popula o passo-conclusão', () => {
  it('AC1: plano de 3 passos ok → só o ÚLTIMO StepResult tem concludes===true', async () => {
    const results = await executeScenario(PLAN_3STEP, makeActions())
    expect(results).toHaveLength(3)
    expect(results[2].concludes).toBe(true)
    expect(results[0].concludes).toBeFalsy()
    expect(results[1].concludes).toBeFalsy()
  })

  it('AC2: passo de navegação cujo resultado expõe url → observedIdentity carrega essa url', async () => {
    const actions = makeActions({
      navigate: vi.fn().mockResolvedValue({ ok: true, url: 'http://ex.com/import/confirm', title: 'C' }),
    })
    const results = await executeScenario(PLAN_NAVIGATE, actions)
    expect(results[0].observedIdentity).toBe('http://ex.com/import/confirm')
  })

  it('AC4: plano vazio → [] e nenhum StepResult marcado concludes', async () => {
    const results = await executeScenario({ steps: [], unresolved: 0 }, makeActions())
    expect(results).toEqual([])
    expect(results.some((r) => r.concludes)).toBe(false)
  })

  // AC3 (corrigida — a premissa original de reconciliação era falsa): o veredito de cenário
  // (passed|failed|inconclusive) é DOMÍNIO SEPARADO do journey-run/harness (pass|fail|error).
  // Verificado por grep: NÃO existe bridge ScenarioVerdict→JourneyRunVerdict em src/plugins/browser.
  // Logo 'inconclusive' NÃO requer mudança nesses schemas. Este teste guarda a separação.
  it('AC3: os schemas journey-run/harness são domínio de verdict separado — não carregam veredito de cenário', () => {
    expect(JourneyRunVerdictSchema.options).not.toContain('inconclusive')
    expect(JourneyRunVerdictSchema.options).not.toContain('passed')
    expect(HarnessRunVerdictSchema.options).not.toContain('inconclusive')
    expect(HarnessRunVerdictSchema.options).not.toContain('passed')
    // Vocabulário próprio, disjunto do oráculo de cenário:
    expect(JourneyRunVerdictSchema.options).toContain('pass')
  })
})
