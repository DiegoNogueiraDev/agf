...
describe('ToolCallLog', () => {
  ...
  describe('getAllCalls', () => {
    it('returns calls across all nodes for a project, most recent first', () => {
      log.record('proj_1', 'node_1', 'analyze')
      log.record('proj_1', 'node_2', 'search')
      const calls = log.getAllCalls('proj_1')
      expect(calls).toHaveLength(2)
      expect(calls[0].toolName).toBe('search')
      expect(calls[1].toolName).toBe('analyze')
    })

    it('does not return calls from other projects', () => {
      log.record('proj_1', null, 'analyze')
      log.record('proj_2', null, 'search')
      const calls = log.getAllCalls('proj_1')
      expect(calls).toHaveLength(1)
      expect(calls[0].toolName).toBe('analyze')
    })
    ...
    it('respects the limit argument', () => {
      log.record('proj_1', null, 'a')
      log.record('proj_1', null, 'b')
      log.record('proj_1', null, 'c')
      const calls = log.getAllCalls('proj_1', 2)
      expect(calls).toHaveLength(2)
    })

    it('returns empty array for a project with no calls', () => {
      expect(log.getAllCalls('proj_empty')).toEqual([])
    })
  })
