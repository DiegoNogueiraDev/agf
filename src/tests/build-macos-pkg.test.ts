...
// Derive the version from package.json — the script names the .pkg after it, so a hardcoded
// version breaks on every bump (node_3c6a66fd217b: was pinned to 0.20.6).
const VERSION = JSON.parse(readFileSync(join(ROOT, 'package.json'), 'utf8')).version as string
const PKG_PATH = join(ROOT, 'dist-packages', `agf-${VERSION}-darwin-arm64.pkg`)
...
afterAll(() => {
  ...
  // Remove only THIS test's artifact, not the shared dist-packages/ dir — the windows-installer
  // test builds into the same dir under a parallel vitest worker, and a `rm -rf dist-packages`
  // here races its build (the "Command failed: build-macos-pkg.sh" flake).
  rmSync(PKG_PATH, { force: true })
})
