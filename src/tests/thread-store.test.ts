...
import { compressFile } from '../core/thread-store/rollout-compression.js'
...
describe('LocalThreadStore', () => {
  ...
  describe('appendItems / loadHistory', () => {
    ...
    it('should transparently load history from a compacted (.jsonl.gz) rollout file', async () => {
      await store.createThread({
        id: 'gz-thread',
        source: 'cli',
        modelProvider: 'openai',
      ...
      await store.appendItems({
        id: 'gz-thread',
        items: [{ kind: 'user_message', content: 'cold storage' }],
      })

      const jsonlPath = join(tmpDir, 'sessions', 'rollout-gz-thread.jsonl')
      const gzPath = `${jsonlPath}.gz`
      compressFile(jsonlPath, gzPath)
      unlinkSync(jsonlPath)

      const history = await store.loadHistory({ id: 'gz-thread' })
      expect(history).toHaveLength(1)
      expect(history[0]!.kind).toBe('user_message')
      expect(history[0]!.content).toBe('cold storage')
    })
  })
