...
describe('HookRegistry', () => {
  ...
  it('dispatch runs only handlers matching the event channel', async () => {
    const h1 = vi.fn()
    const h2 = vi.fn()
    registry.register({ id: 'h1', channel: 'session:start', handler: h1 })
    registry.register({ id: 'h2', channel: 'task:pre-execute', handler: h2 })
  ...
  it('unregister also removes circuit state', () => {
  ...
  it('list returns empty array when no handlers registered', () => {
    expect(registry.list()).toEqual([])
  })

  it('handlers execute in priority order (ascending)', async () => {
    const calls: number[] = []
    registry.register({
      id: 'p2',
  ...
  it('attachStatsStore enables stats recording post-construction', async () => {
    const reg2 = new HookRegistry({ timeoutMs: 200 })
    const handler = vi.fn().mockResolvedValue(undefined)
    reg2.register({ id: 'late-bound', channel: 'session:start', handler })
    reg2.attachStatsStore(statsStore)

    await reg2.dispatch(makeEvent('session:start'))

    const stats = statsStore.get('late-bound')
    expect(stats).toBeDefined()
    expect(stats!.callCount).toBe(1)
  })

  it('stats store records call count and durations', async () => {
  ...
  it('dispatch without matching channel handlers does nothing', async () => {
    registry.register({ id: 'h', channel: 'session:start', handler: vi.fn() })
    await expect(registry.dispatch(makeEvent('task:pre-execute'))).resolves.toBeUndefined()
  })

  it('default options use sensible defaults', () => {
