...
describe('aiCompress', () => {
  ...
  describe('command-owned key exemption', () => {
    ...
    // node_138a23eaa5f0 — `agf certainty --explain` exists to make the MEANS
    // explicit; `rationale` (the "why this pillar makes done trustworthy") IS
    // its payload. Stripped as generic noise, the whole feature ships invisible.
    it('preserves nested rationale for the certainty command', () => {
      const result = aiCompress(
        ok({ explain: [{ key: 'code_on_disk', rationale: 'sem arquivo físico o done é alucinação' }] }, 'certainty'),
      )
      const data = result.data as Record<string, unknown>
      const rows = data.explain as Record<string, unknown>[]
      expect(rows[0].rationale).toBe('sem arquivo físico o done é alucinação')
    })

    it('still strips rationale as noise for non-owning commands (gaps)', () => {
      const result = aiCompress(ok({ items: [{ id: 'g1', rationale: 'drill-down noise' }] }, 'gaps'))
      const data = result.data as Record<string, unknown>
