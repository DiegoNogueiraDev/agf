...
describe('skillCommand', () => {
  ...
  it('wires every skill subcommand', () => {
    // Assert the NAMES, not a bare count: a count tells you the number changed but
    // never which subcommand appeared or vanished, which is the thing you need to
    // know when this goes red.
    const names = skillCommand()
      .commands.map((c) => c.name())
      .sort()
    expect(names).toEqual(
      ['create', 'disable', 'discover', 'enable', 'install', 'list', 'new', 'propose', 'show'].sort(),
    )
  })
})
...
describe('agf skill new — name validation (node_wire_8416c7ac5606 — skill-scaffolder wire)', () => {
  ...
  it('a valid kebab-case name scaffolds into a root the list command scans (regression)', async () => {
    // MUDANÇA DE CONTRATO (node_a287463cb115, ADR no nó): `-d/--dir` era o
    // DESTINO e passou a ser a RAIZ DO PROJETO, o mesmo sentido que tem em
    // `skill list`. Antes, criar e listar com a mesma raiz devolvia lista
    // vazia. Quem precisa escolher o caminho literal usa `--dest`.
    dir = mkdtempSync(join(tmpdir(), 'agf-skill-new-valid-'))
    const result = await run(['new', 'my-valid-skill', '-d', dir])
    expect(result.ok).toBe(true)
    expect(existsSync(join(dir, '.claude', 'skills', 'my-valid-skill', 'SKILL.md'))).toBe(true)
  })

  it('--dest still writes exactly where told — the old capability has a home', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-skill-new-dest-'))
    const result = await run(['new', 'my-dest-skill', '-d', dir, '--dest', join(dir, 'meu', 'caminho')])
    expect(result.ok).toBe(true)
    expect(existsSync(join(dir, 'meu', 'caminho', 'my-dest-skill', 'SKILL.md'))).toBe(true)
  })
