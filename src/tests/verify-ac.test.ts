...
describe('verifyAc', () => {
  ...
  it("GIVEN AC 'returns 200 when payload is valid' and testFiles that pass THEN reported as 'satisfied'", () => {
    // O arquivo precisa EXISTIR: o nome do caso promete "testFiles that pass",
    // e um teste que não está no disco não passa — não é coletado. Enquanto o
    // fixture não o criava, este caso afirmava a aprovação vazia que
    // node_2b9edaf0e59d removeu.
    writeFileSync(join(dir, 'irrelevant.test.ts'), 'export {}\n')
    addNode({
      id: 'node_1',
