...
import { prepareTask, finalizeTask, isAgentDriver, resolveEffectiveLevers } from '../core/autonomy/task-prep.js'
...
import { setLeverEnabled, isLeverEnabled, LOSS_SAFE_BUILD_BUNDLE } from '../core/economy/economy-levers-config.js'
...
describe('prepareTask', () => {
  ...
  let savedAutoEnv: string | undefined

  beforeEach(() => {
    // These tests assert the explicit lever-toggle contract, so pin the agent-driver
    // auto-activation OFF: the test runner itself runs under Claude Code (CLAUDECODE set),
    // which would otherwise enable the loss-safe bundle by default (node_7ee81fd6a5e0) and
    // make "default-off" baselines non-deterministic.
    savedAutoEnv = process.env.AGF_ECONOMY_AUTO
  ...
  afterEach(() => {
    store.close()
    if (savedAutoEnv === undefined) delete process.env.AGF_ECONOMY_AUTO
    else process.env.AGF_ECONOMY_AUTO = savedAutoEnv
  })
...
// All code-agent driver env markers across BOTH detectors — detectActiveCLI's CONFIRMED
// claude/copilot/codex/opencode markers + detectAiFromEnv's 14-agent breadth — plus the economy
// escape hatch. withEnv() clears every one to a known slate so the marker under test is the only
// signal: the test runner itself runs under Claude Code (CLAUDECODE set), which would otherwise
// mask everything. Shared by both describes below.
const DRIVER_ENV_KEYS = [
  // Claude Code (detectActiveCLI)
  'CLAUDECODE',
  'CLAUDE_CODE',
  'CLAUDE_CODE_ENTRYPOINT',
  // GitHub Copilot (detectActiveCLI — confirmed live markers)
  'COPILOT_CLI',
  ...
  'COPILOT_HOME',
  'COPILOT_MODEL',
  // Codex / OpenCode (detectActiveCLI)
  'CODEX',
  'OPENCODE',
  // detectAiFromEnv breadth (assumed markers for the long tail)
  'GITHUB_COPILOT',
  'CODEX_CLI',
  'CURSOR',
  'AIDER',
  ...
  'WARP',
  'CODEIUM',
  // economy auto-activation escape hatch
  'AGF_ECONOMY_AUTO',
]
function withEnv(overrides: Record<string, string | undefined>, fn: () => void): void {
  const saved: Record<string, string | undefined> = {}
  for (const k of DRIVER_ENV_KEYS) saved[k] = process.env[k]
  for (const k of DRIVER_ENV_KEYS) Reflect.deleteProperty(process.env, k)
  ...
  try {
    ...
    for (const k of DRIVER_ENV_KEYS) {
...
describe('economy auto-activation for agent drivers (node_7ee81fd6a5e0)', () => {
  // Minimal, pure config source: no lever persisted → everything default-off.
  const emptySource = { getProjectSetting: (): string | null => null }

  it('AC1: driver detected (CLAUDECODE=1) + no opt-out → every bundle lever enabled', () => {
    withEnv({ CLAUDECODE: '1' }, () => {
      expect(isAgentDriver()).toBe(true)
      const cfg = resolveEffectiveLevers(emptySource)
      for (const k of LOSS_SAFE_BUILD_BUNDLE) expect(isLeverEnabled(cfg, k)).toBe(true)
    })
  })

  it('AC1: the reused detectAiFromEnv marker (CLAUDE_CODE) also counts as a driver', () => {
    withEnv({ CLAUDE_CODE: '1' }, () => {
      expect(isAgentDriver()).toBe(true)
      expect(isLeverEnabled(resolveEffectiveLevers(emptySource), 'ncd_dedup')).toBe(true)
    })
  })

  it('AC2: no driver → bundle levers stay OFF (byte-identical to today)', () => {
    withEnv({}, () => {
      expect(isAgentDriver()).toBe(false)
      const cfg = resolveEffectiveLevers(emptySource)
      for (const k of LOSS_SAFE_BUILD_BUNDLE) expect(isLeverEnabled(cfg, k)).toBe(false)
    })
  })

  it('AC3: escape hatch AGF_ECONOMY_AUTO=0 with a driver present → NOT auto-activated', () => {
    withEnv({ CLAUDECODE: '1', AGF_ECONOMY_AUTO: '0' }, () => {
      expect(isAgentDriver()).toBe(true)
      const cfg = resolveEffectiveLevers(emptySource)
      for (const k of LOSS_SAFE_BUILD_BUNDLE) expect(isLeverEnabled(cfg, k)).toBe(false)
    })
  ...
  it('never touches a non-bundle lever (mdl_select stays off even with a driver)', () => {
    withEnv({ CLAUDECODE: '1' }, () => {
      expect(isLeverEnabled(resolveEffectiveLevers(emptySource), 'mdl_select')).toBe(false)
    })
  })
})

describe('isAgentDriver detects every supported code agent by its REAL marker (node_0896f2d904fc)', () => {
  // The env var each agent ACTUALLY sets — confirmed for the four primary drivers via
  // detectActiveCLI (Copilot=COPILOT_CLI not GITHUB_COPILOT, Codex=CODEX not CODEX_CLI,
  // Claude=CLAUDECODE not CLAUDE_CODE), best-effort for the long tail via detectAiFromEnv.
  const AGENT_MARKERS: ReadonlyArray<readonly [string, string]> = [
  ...
  for (const [name, marker] of AGENT_MARKERS) {
    it(`detects ${name} via ${marker}`, () => {
      withEnv({ [marker]: '1' }, () => {
        expect(isAgentDriver()).toBe(true)
      })
    })
  ...
  it('returns false for a human shell (no agent marker set)', () => {
    withEnv({}, () => {
      expect(isAgentDriver()).toBe(false)
    })
  })
