...
describe('assertTrustedMcpServer', () => {
  ...
  // Regression (node_bc3f3c78cc29): the allowlist must match by ROLE (basename), not by the
  // exact invocation path. A resolved absolute path to node (process.execPath, `which node`)
  // IS node — rejecting it made `agf mcp probe` report reachable:false for a healthy stdio
  // server, because the probe pre-checked basename but connect() re-checked the full path.
  it('accepts an absolute path to an allowed command (basename-matched)', () => {
    expect(() => assertTrustedMcpServer({ command: '/usr/local/bin/node', args: ['s.js'] })).not.toThrow()
    expect(() =>
      assertTrustedMcpServer({ command: '/Users/x/.nvm/versions/node/v24.0.0/bin/node', args: [] }),
    ).not.toThrow()
  })

  it('still rejects an absolute path whose basename is a disallowed command', () => {
    expect(() => assertTrustedMcpServer({ command: '/opt/bin/curl', args: [] })).toThrow()
  })

  it('blocks a shell invoked by absolute path — even if its basename is explicitly allowlisted', () => {
    // Default allowlist: sh/bash aren't allowed at all, so an absolute path is rejected outright.
    expect(() => assertTrustedMcpServer({ command: '/bin/sh', args: [] })).toThrow()
    expect(() => assertTrustedMcpServer({ command: '/usr/bin/bash', args: [] })).toThrow()
    // The shell-specific guard must still bite by basename when sh/bash is explicitly allowed —
    // otherwise `/bin/sh` would slip past the role check into a shell invocation.
    expect(() => assertTrustedMcpServer({ command: '/bin/sh', args: [] }, { allowedCommands: ['sh'] })).toThrow(/shell/)
    expect(() => assertTrustedMcpServer({ command: '/usr/bin/bash', args: [] }, { allowedCommands: ['bash'] })).toThrow(
      /shell/,
    )
  })

  it('matches npx by basename too (absolute path) and still enforces pinning', () => {
    // basename npx → npx rules apply → unpinned spec still rejected (no security regression).
    expect(() => assertTrustedMcpServer({ command: '/usr/bin/npx', args: ['-y', 'evil-pkg'] })).toThrow()
    expect(() =>
      assertTrustedMcpServer({ command: '/usr/bin/npx', args: ['@modelcontextprotocol/server@1.0.0'] }),
    ).not.toThrow()
  })
})
