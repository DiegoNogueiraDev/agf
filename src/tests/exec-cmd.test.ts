...
describe('chainOutcome — envelope reflete falha de sub-comando (node_e3972a535bf6)', () => {
  it('todos os steps ok => outcome ok:true', async () => {
    const { chainOutcome } = await import('../cli/commands/exec-cmd.js')
    const r = chainOutcome([
      { command: 'a', ok: true },
      { command: 'b', ok: true },
    ])
  ...
  it('um step com ok:false => outcome ok:false nomeando os steps que falharam', async () => {
    const { chainOutcome } = await import('../cli/commands/exec-cmd.js')
    const r = chainOutcome([
      { command: 'next', ok: true },
      { command: 'edge add A B', ok: false, error: 'EACCES' },
    ])
    expect(r.ok).toBe(false)
    expect(r.code).toBe('CHAIN_STEP_FAILED')
    expect(r.error).toContain('edge add A B')
  })

  it('lista vazia => ok:true (nada a falhar)', async () => {
    const { chainOutcome } = await import('../cli/commands/exec-cmd.js')
    expect(chainOutcome([]).ok).toBe(true)
  })
})
