...
// node_f106f5820032 — ARESTA DE MODULE-GRAPH (não para uso): este guard lê os
// fontes como STRING, então `vitest --changed`/`related` (que só enxerga o grafo
// de imports) NUNCA o marcava afetado quando um dos arquivos que ele protege
// mudava — um wire phone-home reintroduzido no index.ts passava no blast por-task
// (foi exatamente como aconteceu). Importar os entrypoints de MAIOR risco cria a
// aresta: agora editar index.ts / done-cmd.ts marca este guard como related.
// Import lateral, side-effect-free (o bootstrap do index.ts é gated por
// isCliEntrypoint, que é false sob o test runner).
import '../cli/index.js'
import '../cli/commands/done-cmd.js'
