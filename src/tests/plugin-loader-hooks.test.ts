...
describe('PluginLoader + Extension Hooks', () => {
  ...
  it('registers plugin tools into a PluginToolRegistry when provided (node_wire_054287d213cf)', async () => {
    const builder = new ExtensionRegistryBuilder()
    const registry = new PluginRegistry()
    const loader = new PluginLoader(registry)
    ...
    const manifest: PluginManifest = {
      name: 'tool-plugin',
      version: '1.0.0',
      description: 'Plugin exposing a tool',
      entryPoint: 'test.js',
      capabilities: ['tool'],
    }
    const instance = {
      activate: (ctx: PluginContext) => {
        ctx.registerTool('do-thing', handler)
      },
    }

    await loader.loadPlugin(manifest, instance, builder, toolRegistry)

    expect(toolRegistry.isPluginTool('do-thing')).toBe(true)
    expect(toolRegistry.getPluginForTool('do-thing')).toBe('tool-plugin')
    expect(toolRegistry.list()).toHaveLength(1)
  })

  it('works without a toolRegistry — registerTool stays a no-op (node_wire_054287d213cf)', async () => {
    const builder = new ExtensionRegistryBuilder()
    const registry = new PluginRegistry()
    const loader = new PluginLoader(registry)
    ...
    const manifest: PluginManifest = {
      name: 'no-registry',
      version: '1.0.0',
      description: 'Plugin without a tool registry wired in',
      entryPoint: 'test.js',
      capabilities: ['tool'],
    }
    const instance = {
      activate: (ctx: PluginContext) => {
        ctx.registerTool('ignored', vi.fn())
      },
    }

    await expect(loader.loadPlugin(manifest, instance, builder)).resolves.not.toThrow()
  })
})
