...
describe('submit — finalize parity + token capture (delegate ↔ live unified)', () => {
  ...
  it('AC1: standalone `--tokens <in,out>` records ONE attributed self-reported row (no --result needed)', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-submit-standalone-'))
    seedNode('t_sr')
    await runSubmit(['t_sr', '--tokens', '1200,800', '--dir', dir])

    const store = SqliteStore.open(dir)
    const rows = store
      .getDb()
      .prepare(
        "SELECT input_tokens AS i, output_tokens AS o, provider AS p, node_id AS n FROM llm_call_ledger WHERE node_id = 't_sr'",
      )
      .all() as Array<{ i: number; o: number; p: string; n: string }>
    store.close()
    expect(rows.length).toBe(1)
    expect(rows[0].i).toBe(1200)
    expect(rows[0].o).toBe(800)
    expect(rows[0].n).toBe('t_sr')
    expect(rows[0].p).toBe('delegated') // the external-agent / self-reported marker
  })

  it('AC2: `submit <id>` with neither --tokens nor --result is unchanged — errors, no ledger row', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-submit-noop-'))
    seedNode('t_noop')
    await runSubmit(['t_noop', '--dir', dir]) // no --result, no --tokens

    const store = SqliteStore.open(dir)
    const count = store.getDb().prepare("SELECT COUNT(*) AS c FROM llm_call_ledger WHERE node_id = 't_noop'").get() as {
      c: number
    }
    store.close()
    expect(count.c).toBe(0)
