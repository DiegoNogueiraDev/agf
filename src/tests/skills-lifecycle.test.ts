...
describe('all skills — token-economy invariant', () => {
  ...
  /**
   * A skill teaches token economy in one of two ways, and both are correct.
   *
   * It writes its own section, because its economy is peculiar to it — a builder
   * delegates, a planner never does. Or it points at `_shared.md`, which carries
   * the levers every skill shares. Demanding the section verbatim in all thirty-one
   * would duplicate the same paragraph thirty-one times, and the thirty-second
   * would be the one that drifts.
   *
   * The invariant is that an agent reading the skill can find the levers. Not that
   * every file repeats them.
   */
  const POINTS_AT_SHARED = /_shared\.md/

  for (const { id, path } of skills) {
    it(`${id} teaches token economy, in its own section or by pointer`, () => {
      const content = readFileSync(path, 'utf-8')
      // Builder edition uses "## Economy"; planner uses "## Token Economy".
      const hasOwnSection = /^## (Token Economy|Economy)/m.test(content)
      expect(
        hasOwnSection || POINTS_AT_SHARED.test(content),
        `${id}: no economy section and no pointer to _shared.md`,
      ).toBe(true)
    })

    it(`${id} surfaces the core economy levers, or points where they live`, () => {
      const content = readFileSync(path, 'utf-8')
      if (POINTS_AT_SHARED.test(content)) return // the levers live there, once
      expect(content, `${id}: teaches its own economy but omits --select`).toMatch(/--select/)
      expect(content, `${id}: teaches its own economy but omits retrieve-command`).toMatch(/retrieve-command/)
