...
describe('agf lsp find-symbol (node_wire_491687c71cb4)', () => {
  ...
  it('finds a nested symbol by name path and reports its location', async () => {
    ...
    const result = await run(['find-symbol', '--symbols', symbols, '--path', 'MyClass/myMethod', '--file', 'src/example.ts'])
    expect(result.ok).toBe(true)
    const data = result.data as { found: boolean; symbol?: { name: string; location: { startLine: number; endLine: number } } }
  ...
  it('reports found=false when the name path matches nothing', async () => {
    const symbols = JSON.stringify([{ name: 'MyClass', kind: 'class', file: 'src/example.ts', startLine: 0, endLine: 10 }])
    const result = await run(['find-symbol', '--symbols', symbols, '--path', 'DoesNotExist', '--file', 'src/example.ts'])
    expect(result.ok).toBe(true)
