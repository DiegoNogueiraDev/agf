...
describe('dashboard API', () => {
  ...
  // node_wire_f1c88c12a5ab — wires the dormant AgentRegistry (src/core/store/agent-registry.ts)
  // into the dashboard API so live agent heartbeats are visible over HTTP.
  it('POST /api/v1/agent/register + GET /api/v1/agent/active → registered agent appears as active', async () => {
    const reg = await fetch(`${base}/api/v1/agent/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ agentId: 'agent-1', capabilities: ['read', 'write'] }),
    })
    expect(reg.status).toBe(201)

    const res = await fetch(`${base}/api/v1/agent/active`)
    expect(res.status).toBe(200)
    const body = (await res.json()) as {
      agents: Array<{ agentId: string; status: string; capabilities: string[] }>
    }
    const entry = body.agents.find((a) => a.agentId === 'agent-1')
    expect(entry?.status).toBe('active')
    expect(entry?.capabilities).toEqual(['read', 'write'])
  })

  it('POST /api/v1/agent/register → 400 when agentId is missing', async () => {
    const res = await fetch(`${base}/api/v1/agent/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('DELETE /api/v1/agent/:agentId → removes the agent from the active list', async () => {
    await fetch(`${base}/api/v1/agent/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ agentId: 'agent-2', capabilities: [] }),
    })
    const del = await fetch(`${base}/api/v1/agent/agent-2`, { method: 'DELETE' })
    expect(del.status).toBe(204)

    const res = await fetch(`${base}/api/v1/agent/active`)
    const body = (await res.json()) as { agents: Array<{ agentId: string }> }
    expect(body.agents.some((a) => a.agentId === 'agent-2')).toBe(false)
  })
})
