...
describe('codeCommand contract-drift', () => {
  let dir: string
  ...
  async function runDrift(args: string[]): Promise<{ ok: boolean; data?: Record<string, unknown>; code?: string }> {
    const out: string[] = []
    const originalWrite = process.stdout.write.bind(process.stdout)
    process.stdout.write = ((chunk: unknown) => {
      out.push(String(chunk))
    ...
    try {
      await codeCommand().parseAsync(['contract-drift', ...args, '-d', dir], { from: 'user' })
    } finally {
      process.stdout.write = originalWrite
    }
    return JSON.parse(out.join('').trim())
  ...
  it('returns NOT_FOUND when the node does not exist', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-contract-drift-'))
    const store = SqliteStore.open(dir)
    store.initProject('contract-drift-test')
    store.close()

    const file = join(dir, 'impl.ts')
    writeFileSync(file, 'export class Foo {}\n')

    const envelope = await runDrift(['node_missing', file])
    expect(envelope.ok).toBe(false)
    expect(envelope.code).toBe('NOT_FOUND')
  })

  it('flags removed_from_code as critical drift when a graph-declared method is missing from the implementation', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-contract-drift-'))
    const store = SqliteStore.open(dir)
    store.initProject('contract-drift-test')
    const now = new Date().toISOString()
    store.insertNode({
      ...
      status: 'backlog',
      priority: 3,
      metadata: { methods: { doThing: '(id: string): void', missingMethod: '(): void' } },
      acceptanceCriteria: [],
      tags: [],
      createdAt: now,
      updatedAt: now,
    ...
    const envelope = await runDrift(['node_contract', file])
    expect(envelope.ok).toBe(true)
    const report = envelope.data as { critical: boolean; changes: Array<{ method: string; type: string }> }
    expect(report.critical).toBe(true)
    expect(report.changes.some((c) => c.method === 'missingMethod' && c.type === 'removed_from_code')).toBe(true)
  })

  it('reports no drift when graph and code signatures match', async () => {
    dir = mkdtempSync(join(tmpdir(), 'agf-contract-drift-'))
    const store = SqliteStore.open(dir)
    store.initProject('contract-drift-test')
    const now = new Date().toISOString()
    store.insertNode({
      ...
      status: 'backlog',
      priority: 3,
      metadata: { methods: { doThing: '(id: string): void' } },
      acceptanceCriteria: [],
      tags: [],
      createdAt: now,
      updatedAt: now,
    ...
    const envelope = await runDrift(['node_contract', file])
    expect(envelope.ok).toBe(true)
    const report = envelope.data as { hasDrift: boolean }
