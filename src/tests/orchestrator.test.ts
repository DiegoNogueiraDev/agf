...
describe('nextDeliveryAction — orquestração determinística (#O1)', () => {
  ...
  it('nodes sem requirements nem ready tasks → escalate (NÃO re-importa — node_1ffe8eb1c7b9)', () => {
    // Antes re-importava para sempre (import só quando o grafo está vazio agora).
    expect(nextDeliveryAction(state({ totalNodes: 3, hasRequirements: false })).action).toBe('escalate')
  })

  it('nodes com ready tasks mas sem requirement-nodes → implement (PRD que gera tasks diretas)', () => {
