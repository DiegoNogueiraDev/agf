...
describe('scripts/gen-packages.sh — covers every cross-compile target', () => {
  ...
  it('builds linux x64/arm64, darwin x64, and win32 x64 (darwin-arm64 is native, built elsewhere)', () => {
    ...
    // Windows goes through its own helper since v0.24.0: it is the only target
    // DISTRIBUTED as a tarball, so it must be built once per supported Node ABI
    // rather than once for the build host. Covered in depth in node-abi.test.ts.
    expect(shSrc).toContain('build_windows_all_abis')
  })
})
