...
describe('flow-config', () => {
  describe('resolveFlowConfig', () => {
    ...
    it('defaults maxPinnedPullIn to 8 (risk node_db3cf9a2e2b1 tuning)', () => {
      const source = { getProjectSetting: (_key: string) => null }
      expect(resolveFlowConfig(source).maxPinnedPullIn).toBe(8)
    })

    it('honours an explicit maxPinnedPullIn override', () => {
      const source = { getProjectSetting: (_key: string) => '{"maxPinnedPullIn":3}' }
      expect(resolveFlowConfig(source).maxPinnedPullIn).toBe(3)
    })
  })
