 ...
 * AC5: generateCreativeFiles: returns [] when generated content has a banned identifier (ast-source-validator)
 */
...
describe('generateCreativeFiles', () => {
  ...
  it('AC5: returns [] when a generated file contains a banned identifier (eval)', async () => {
    const plan = { files: [{ path: 'src/auth.ts', content: 'eval("1+1")' }] }
    const gen = vi.fn().mockResolvedValue(`\`\`\`json\n${JSON.stringify(plan)}\n\`\`\``)
    const result = await generateCreativeFiles(NODE, GAP, gen)
    expect(result).toEqual([])
  })

  it('AC5: returns [] when any file in a multi-file plan is unsafe', async () => {
    const plan = {
      files: [
        { path: 'src/a.ts', content: 'export const a = 1' },
        { path: 'src/b.ts', content: 'const fs = require("fs")' },
      ],
    }
    const gen = vi.fn().mockResolvedValue(`\`\`\`json\n${JSON.stringify(plan)}\n\`\`\``)
    const result = await generateCreativeFiles(NODE, GAP, gen)
  ...
  it('AC5: returns files when all generated content is safe', async () => {
    const plan = { files: [{ path: 'src/auth.ts', content: 'export const auth = {}' }] }
    const gen = vi.fn().mockResolvedValue(`\`\`\`json\n${JSON.stringify(plan)}\n\`\`\``)
    const result = await generateCreativeFiles(NODE, GAP, gen)
    expect(result).toHaveLength(1)
  })
