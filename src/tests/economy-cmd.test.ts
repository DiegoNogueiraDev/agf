...
describe('agf economy ab-lever — CLI surface for core/economy/lever-ab-harness.ts (WIRE node_wire_30e01765b5a0)', () => {
  let store: SqliteStore

  beforeEach(() => {
    store = SqliteStore.open(':memory:')
    store.initProject('economy-ab-lever-test')
    vi.mocked(openStoreOrFail).mockReturnValue(store as unknown as ReturnType<typeof openStoreOrFail>)
  })
  ...
  // AC: GIVEN valid levers and tasks WHEN no live executor is wired THEN the envelope
  // reports mode:'delegated' with a reason — never a silent zero (runLeverAb's own contract).
  it('returns mode delegated when no live executor is wired', async () => {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    })
    await economyCommand().parseAsync(['ab-lever', 'memory_salience', 'task-1,task-2'], { from: 'user' })
    spy.mockRestore()
    const env = lastEnvelope(out)
    expect(env.ok).toBe(true)
    const data = env.data as { mode: string; reason?: string }
    expect(data.mode).toBe('delegated')
    expect(data.reason).toBeTruthy()
  })

  // AC: GIVEN an unknown lever name THEN the command fails with UNKNOWN_LEVER, never silently ignoring it
  it('fails with UNKNOWN_LEVER for an unrecognized lever', async () => {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    })
    await economyCommand().parseAsync(['ab-lever', 'not_a_real_lever', 'task-1'], { from: 'user' })
    spy.mockRestore()
    const env = lastEnvelope(out)
    expect(env.ok).toBe(false)
    expect(env.code).toBe('UNKNOWN_LEVER')
  })

  // AC: GIVEN multiple comma-separated levers THEN the command accepts all of them in one envelope
  it('accepts multiple comma-separated levers', async () => {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    })
    await economyCommand().parseAsync(['ab-lever', 'memory_salience,ncd_dedup', 'task-1'], { from: 'user' })
    spy.mockRestore()
    const env = lastEnvelope(out)
    expect(env.ok).toBe(true)
  })
...
describe('agf economy measure-input — CLI surface for core/economy/lever-input-measure.ts (WIRE node_wire_1ec228afd1d5)', () => {
  let store: SqliteStore
  let nodeId: string

  beforeEach(() => {
    store = SqliteStore.open(':memory:')
    store.initProject('economy-measure-input-test')
    const now = new Date().toISOString()
    nodeId = 'node_measure_input_target'
    const node: GraphNode = {
      id: nodeId,
      title: 'Implementa o coletor de metricas',
      type: 'task',
      status: 'backlog',
      priority: 3,
      acceptanceCriteria: [],
  ...
  // AC: GIVEN a valid lever and node THEN the envelope reports {lever, before, after, saved}
  // — the same shape measureInputLever already returns, now reachable from the CLI.
  it('returns the input-cut measurement for a single lever', async () => {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    })
    await economyCommand().parseAsync(['measure-input', 'ncd_dedup', nodeId], { from: 'user' })
    spy.mockRestore()
    const env = lastEnvelope(out)
    expect(env.ok).toBe(true)
    const data = env.data as { lever: string; before: number; after: number; saved: number }
    expect(data.lever).toBe('ncd_dedup')
    expect(typeof data.before).toBe('number')
    expect(typeof data.after).toBe('number')
    expect(data.saved).toBe(data.before - data.after)
  })
  ...
  it('fails with UNKNOWN_LEVER for an unrecognized lever', async () => {
    ...
    await economyCommand().parseAsync(['measure-input', 'not_a_real_lever', nodeId], { from: 'user' })
    spy.mockRestore()
    const env = lastEnvelope(out)
    expect(env.ok).toBe(false)
    expect(env.code).toBe('UNKNOWN_LEVER')
  ...
  // AC: GIVEN a node id that does not exist THEN the command fails with NODE_NOT_FOUND instead of throwing raw
  it('fails with NODE_NOT_FOUND for a missing node', async () => {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    })
    await economyCommand().parseAsync(['measure-input', 'ncd_dedup', 'node_does_not_exist'], { from: 'user' })
    spy.mockRestore()
    const env = lastEnvelope(out)
    expect(env.ok).toBe(false)
    expect(env.code).toBe('NODE_NOT_FOUND')
  })
})
