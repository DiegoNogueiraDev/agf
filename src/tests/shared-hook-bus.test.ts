...
describe('self-healing listener auto-registration (node_wire_3b0469edd862)', () => {
  let dir: string
  let cwdSpy: ReturnType<typeof vi.spyOn>

  beforeEach(() => {
    _resetSharedHookBus()
    dir = mkdtempSync(join(tmpdir(), 'agf-shared-hook-bus-healing-'))
    cwdSpy = vi.spyOn(process, 'cwd').mockReturnValue(dir)
  })
  ...
  it('writes a healing memory file when error:detected is emitted on the shared bus', () => {
    getSharedHookBus()
    const graphBus = _getSharedGraphBus()
    expect(graphBus).toBeInstanceOf(GraphEventBus)
    ...
    graphBus!.emit({
      ...
      payload: {
        toolName: 'Bash',
        errorMessage: 'database connection failed: SQLITE_ERROR',
        errorCategory: 'database-error',
        errorHash: 'abc123def456',
      },
    } as unknown as GraphEvent)

    const memoryPath = join(dir, 'workflow-graph', 'memories', 'healing-database-error-abc123def456.md')
    expect(existsSync(memoryPath)).toBe(true)
  })

  it('does not re-register the listener across repeated getSharedHookBus calls', () => {
    getSharedHookBus()
    getSharedHookBus()
    const graphBus = _getSharedGraphBus()
    ...
    graphBus!.emit({
      ...
      payload: {
        toolName: 'Bash',
        errorMessage: 'build failed: compile error',
        errorCategory: 'build-error',
        errorHash: 'def456abc789',
      },
    } as unknown as GraphEvent)

    const memoryPath = join(dir, 'workflow-graph', 'memories', 'healing-build-error-def456abc789.md')
    expect(existsSync(memoryPath).valueOf()).toBe(true)
    const content = readFileSync(memoryPath, 'utf-8')
    // Content written exactly once (no duplicate handler double-writing)
    expect(content.match(/# Self-Healing: build-error/g)?.length).toBe(1)
  })
})
