...
describe('agf mcp probe (node_wire_c94b2e286642)', () => {
  ...
  // AC6 (node_wire_e4648137ae68 — registry-allowlist wire): a non-allowlisted command is rejected
  // BEFORE any spawn is attempted, instead of shelling out to an untrusted binary.
  it('rejects a non-allowlisted --command without spawning it', async () => {
    ...
    expect(result.ok).toBe(false)
    expect(result.code).toBe('UNTRUSTED_MCP_SERVER')
  })

  // AC7: an unpinned npx package spec is rejected before spawning.
  it('rejects an unpinned npx package spec without spawning it', async () => {
    const result = await run([
      'probe',
      '--name',
      'untrusted-npx',
      ...
      'npx',
      '--args',
      '-y,some-untrusted-package',
    ])
    expect(result.ok).toBe(false)
    expect(result.code).toBe('UNTRUSTED_MCP_SERVER')
  })

  // AC8: an allowlisted command still spawns normally — the gate does not regress the happy path.
  it('still allows an allowlisted --command (basename-matched) to spawn', async () => {
    const result = await run(['probe', '--name', 'local-server', '--command', process.execPath, '--args', '--version'])
    expect(result.ok).toBe(true)
    const data = result.data as { reachable: boolean }
    expect(data.reachable).toBe(true)
  })
})
