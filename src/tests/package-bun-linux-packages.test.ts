...
describe('package-bun.yml — Linux .deb/AppImage packaging is wired, non-fatally', () => {
  it('runs build-linux-packages.sh after the standalone binaries are built', () => {
    const buildIdx = workflow.indexOf('Build standalone binaries')
  ...
  it('uploads the .deb/.AppImage as a separate, non-blocking artifact step', () => {
    const uploadIdx = workflow.indexOf('dist-packages')
    expect(uploadIdx).toBeGreaterThan(-1)
    const artifactStep = workflow.slice(workflow.lastIndexOf('name:', uploadIdx))
    expect(artifactStep).toMatch(/if-no-files-found:\s*warn/)
  })
