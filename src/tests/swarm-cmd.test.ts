...
describe('agf swarm command (#node_9a1d912ce68d)', () => {
  ...
  it('bench measures serial vs. batch wall-clock over the same claimed set (node_wire_a54279a32eab)', async () => {
    const store = SqliteStore.open(dir)
    const now = new Date().toISOString()
    for (const id of ['bench_a', 'bench_b']) {
      store.insertNode({
        id,
        type: 'task',
        title: id,
    ...
    const env = await run(['bench', '-d', dir, '-k', '2', '--delay-ms', '20'])
    expect(env.ok).toBe(true)
    const data = env.data as { tasks: number; k: number; serialMs: number; batchMs: number; speedup: number }
    expect(data.tasks).toBe(2)
    expect(data.serialMs).toBeGreaterThan(0)
    expect(data.batchMs).toBeGreaterThan(0)
  })

  it('bench on an empty backlog returns explicit zeros, no invented speedup', async () => {
    const env = await run(['bench', '-d', dir, '-k', '2'])
    expect(env.ok).toBe(true)
    const data = env.data as { tasks: number; speedup: number }
    expect(data.tasks).toBe(0)
    expect(data.speedup).toBe(0)
  })
})
