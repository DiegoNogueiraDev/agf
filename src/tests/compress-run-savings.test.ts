...
describe('surface attribution (F2.T2 node_41efe88b1dcd)', () => {
  it('disparo via compress run grava surface=hook', () => {
    const db = makeDb()
    recordCompressRunSavings(db, { tokensBefore: 200, tokensAfter: 100, saved: 100 })
    const row = db.prepare('SELECT surface FROM economy_lever_ledger LIMIT 1').get() as { surface: string }
