...
describe('agf harness --violations includes Pareto dimension priority (node_wire_e64f4522d25a)', () => {
  ...
  it('priority reflete EXATAMENTE as dimensões abaixo de 70 (drift-proof, não assume o score vivo)', async () => {
    const out: string[] = []
    const spy = vi.spyOn(process.stdout, 'write').mockImplementation((chunk: unknown) => {
      out.push(String(chunk))
      return true
    ...
    const envelope = lastEnvelope(out)
    const data = envelope.data as {
      priority?: Array<{ dimension: string; score: number }>
      breakdown: Record<string, { score: number }>
    }
    // Contrato invariante (não depende de o repo estar all-healthy hoje):
    // a chave priority está presente sse existe ≥1 dimensão < 70, e lista só essas.
    const belowThreshold = Object.entries(data.breakdown)
      .filter(([, v]) => v.score < 70)
      .map(([k]) => k)
      .sort()
    if (belowThreshold.length === 0) {
      ...
      expect(data.priority).toBeDefined()
      for (const p of data.priority ?? []) expect(p.score).toBeLessThan(70)
      expect((data.priority ?? []).map((p) => p.dimension).sort()).toEqual(belowThreshold)
    }
  })
})
