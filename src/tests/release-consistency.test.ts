...
describe('checkReleaseConsistency', () => {
  it('flags the stale windows channel as divergent (0.20.4 vs target 0.20.5)', async () => {
    const target = '0.20.5'
    const fetch = stubVersions({
      ...
      'agf-windows-x64.exe': '0.20.4',
    })

    const result = await checkReleaseConsistency(target, fetch)

    expect(result.ok).toBe(false)
    expect(result.divergent).toEqual(['agf-windows-x64.exe'])
  ...
  it('marks a channel missing (not throwing) when its fetch yields null', async () => {
    ...
    const fetch = stubVersions({
      ...
      'agf-linux-x64': '0.20.5',
      'agf-linux-arm64': '0.20.5',
      'agf-windows-x64.exe': null, // fetch failed / asset absent
    })

    const result = await checkReleaseConsistency(target, fetch)

    expect(result.ok).toBe(false)
    expect(result.missing).toEqual(['agf-windows-x64.exe'])
    expect(result.channels.find((c) => c.asset === 'agf-windows-x64.exe')?.status).toBe('missing')
  })
  ...
  it('exposes the exact published asset set (windows carries the .exe extension)', () => {
    expect(PUBLISHED_ASSETS).toContain('agf-windows-x64.exe')
    expect(PUBLISHED_ASSETS).toContain('agf-darwin-arm64')
    expect(PUBLISHED_ASSETS).toContain('agf-linux-arm64')
    expect(PUBLISHED_ASSETS).toHaveLength(5)
  })
})
...
const BUILDINFO_FIXTURE = JSON.stringify({
  ...
  targets: [
    ...
    { out: 'agf-windows-x64.exe', sha256: 'bbb' },
  ],
})
...
describe('parseExpectedShas', () => {
  it('extracts asset→sha256 pairs from BUILDINFO targets', () => {
    expect(parseExpectedShas(BUILDINFO_FIXTURE)).toEqual([
      ...
      { asset: 'agf-windows-x64.exe', sha256: 'bbb' },
    ])
  })
...
describe('checkServedIntegrity (consumer-mode: HTTPS-served bytes vs BUILDINFO)', () => {
  const expected = [
    { asset: 'agf-windows-x64.exe', sha256: '2bb210f6' },
    { asset: 'agf-darwin-arm64', sha256: '47b98256' },
  ]

  it('flags the CDN-stale windows channel when the served sha differs from BUILDINFO', async () => {
    const fetch = stubServedShas({ 'agf-windows-x64.exe': 'deadbeef', 'agf-darwin-arm64': '47b98256' })
  ...
  it('returns ok=true when every served sha matches BUILDINFO', async () => {
    const fetch = stubServedShas({ 'agf-windows-x64.exe': '2bb210f6', 'agf-darwin-arm64': '47b98256' })
  ...
  it('marks a channel missing (no throw) when the HTTPS fetch fails/rejects', async () => {
    const fetch: ServedShaSource = async (asset) => {
      if (asset === 'agf-windows-x64.exe') throw new Error('CDN 5xx')
      return '47b98256'
    }
