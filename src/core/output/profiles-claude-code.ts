...
export const PROFILE_CLAUDE_CODE: Record<string, CommandProfile> = {
  ...
  metrics: {
    select: ['data.totals', 'data.avgTokensPerTask', 'data.costPerSuccess', 'data.succeeded'],
  },
  savings: {
    select: ['data.totals', 'data.savingsRate', 'data.totalSaved'],
  },
  ...
  'retrieve-command': {
    select: ['data.query', 'data.command', 'data.economy'],
  },
  'montar-output': {
    select: ['data.decision', 'data.scaffold', 'data.language'],
  },
