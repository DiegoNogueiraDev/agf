...
export const PROFILE_COPILOT: Record<string, CommandProfile> = {
  ...
  check: {
    ...
    select: ['data.taskId', 'data.surface_proof', 'data.dodScore', 'data.dodGrade', 'data.next.id', 'data.next.title'],
  },
  submit: {
  ...
  metrics: {
    ...
    select: ['data.totals', 'data.surfaceGate', 'data.avgTokensPerTask', 'data.costPerSuccess'],
  },
  decompose: {
