...
/**
 * Chaves podadas SEMPRE — o motivo delas não é volume (config verbosa, detalhe
 * que não interessa a agente), então vale para escalar também.
 */
export const ALWAYS_NOISE_KEYS = new Set([
...
/**
 * Chaves podadas SÓ quando são volume (array ou objeto).
 *
 * PORQUÊ o grupo existe (node_2872d221f92d): estes nomes entraram na lista para
 * cortar detalhe pesado — a quebra por task do relatório de economia, o catálogo
 * de levers. Mas a poda é por NOME em qualquer profundidade, então o escalar
 * `tasks: 4` do envelope de `agf swarm bench` sumia junto, deixando um speedup
 * sem denominador: 3.85× sobre 4 tasks ou sobre 400 são leituras diferentes.
 *
 * A regra: a lista existe para cortar VOLUME, e um escalar não é volume. Nome
 * genérico em deny-list global é bomba-relógio — cada novo comando que use a
 * palavra `tasks` para um número perderia o campo em silêncio.
 */
export const BULK_ONLY_NOISE_KEYS = new Set([
  // Financial drill-down (o motivo original de cada um destes)
  'tasks',
  'commands',
  'levers',
  'byTask',
...
/** Volume = coleção. Um escalar carrega um fato; uma coleção carrega tokens. */
function isBulk(value: unknown): boolean {
  return Array.isArray(value) || (value !== null && typeof value === 'object')
}

/** A chave deve ser podada NESTE valor? */
function isNoise(key: string, value: unknown, owned: ReadonlySet<string>): boolean {
  if (owned.has(key)) return false
  if (ALWAYS_NOISE_KEYS.has(key)) return true
  return BULK_ONLY_NOISE_KEYS.has(key) && isBulk(value)
}
...
const COMMAND_OWNED_KEYS: Record<string, ReadonlySet<string>> = {
  ...
  // `agf certainty --explain` exists precisely to make the MEANS explicit: each
  // pillar's `rationale` (why it makes the done trustworthy) IS the payload the
  // user asked for. As generic noise it would be stripped and the feature would
  // ship invisible — the exact failure this table was created to prevent.
  certainty: new Set(['rationale']),
}
