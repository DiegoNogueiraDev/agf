...
export const GAP_KINDS = [
  ...
  'phantom_done',
  // Efeito-no-driver (F2.T3): lever ON com superfície driver-facing declarada mas
  // zero linhas de economia nela nas últimas sessões — wired ≠ firing-no-driver.
  'driver_boundary_missing',
  // Código que entrou sem nó no grafo (node_9bb2e60a6390) — a regra de ouro
  // 'sem node, sem código' finalmente com detector.
  'orphan_commit',
  // AC de pai nao coberta com TODOS os filhos done — nao e 'redistribua',
  // e 'pode nunca ter sido entregue' (node_9844108d6e9e).
  'ac_delivery_doubt',
  // Plan-time phantom: task backlog/ready declara "EXPAND src/X.ts" no description
  // mas o arquivo não existe no disco — pointer morto, severidade RECOMMENDED.
  'phantom_pointer',
  // Design by Contract (Meyer 1992): task de boundary (rota/api/comando/hook/WIRE:)
  // sem edge implements/consumes para um node contract — o executor inventa o shape.
  'contract_coverage',
] as const
