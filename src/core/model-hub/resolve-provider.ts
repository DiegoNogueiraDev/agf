...
import { CachingModelAdapter, responseCacheEnabled, type SemanticCacheWire } from './caching-model-adapter.js'
...
export function resolveProviderAdapter(
  ...
  if (cache && responseCacheEnabled(env)) {
    const caching = new CachingModelAdapter(adapter, cache, { providerId: primaryId, semantic })
