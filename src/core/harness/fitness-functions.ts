...
export function checkBarrelIntegrity(dirs: DirectoryInfo[]): FitnessCheckResult {
  ...
  for (const dir of dirs) {
    ...
    // node_30c368c6e1a5: an index.ts opening with a shebang is a bin ENTRYPOINT
    // (e.g. src/swarming/index.ts: `#!/usr/bin/env node`), not a barrel. Re-
    // exporting siblings there would run the bootstrap as an import side effect —
    // exempt it instead of demanding barrel re-exports it must not have.
    if (dir.indexContent.startsWith('#!')) {
      checkedDirs++
      continue
    }
