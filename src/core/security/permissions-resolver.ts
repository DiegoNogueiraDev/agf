...
/**
 * Resolve a candidate path to a normalized absolute path. Absolute paths are ALSO run
 * through resolvePath so that `..`/`.` segments collapse — without this, `/tmp/../etc/passwd`
 * kept its literal form and matched a `/tmp` sandbox entry while actually pointing at
 * `/etc/passwd` (traversal sandbox escape, node_f0a947d02eb7).
 */
function resolveCandidatePath(path: string, cwd: string): string | null {
  try {
    return path.startsWith('/') ? resolvePath(path) : resolvePath(cwd, path)
  } catch {
    return null
  }
}
...
/**
 * True when `absPath` is `base` itself or lives strictly inside it. Uses a `/` boundary so a
 * sibling that merely shares the prefix (`/tmpevil` vs `/tmp`, `proj-secrets` vs `proj`) does
 * NOT match — a plain startsWith let those escape the sandbox (node_f0a947d02eb7).
 */
function isPathWithin(absPath: string, base: string): boolean {
  if (base === '/') return absPath.startsWith('/')
  const normBase = base.endsWith('/') ? base.slice(0, -1) : base
  return absPath === normBase || absPath.startsWith(normBase + '/')
}
...
function entryMatchesPath(entry: FileSystemSandboxEntry, absPath: string): boolean {
  const p = entry.path
  if (p.type === 'Path') {
    return isPathWithin(absPath, p.path)
  }
  ...
  if (p.type === 'Special') {
    ...
    if (p.value === 'Minimal') return absPath.startsWith('/') && !isPathWithin(absPath, '/dev')
    if (p.value === 'Tmpdir') return isPathWithin(absPath, '/tmp')
    if (p.value === 'SlashTmp') return isPathWithin(absPath, '/tmp')
