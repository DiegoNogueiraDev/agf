...
/**
 * Shell separators that introduce a NEW command: `;` `&&` `||` `|` `&`. A prefix Allow/Prompt
 * rule that matches the start of a chained command must not grant it — the appended command
 * was never vetted. Only Forbidden rules match a chain broadly. (node_7cd9e73255b1)
 */
const HAS_COMMAND_SEPARATOR_RE = /(?:&&|\|\||;|\||&)/
    ...
    for (const rule of this.rules) {
      ...
      if (rule.type === 'prefix') {
        ...
        if (trimmed === prefixStr || trimmed.startsWith(prefixStr + ' ')) {
          // An Allow/Prompt prefix must NOT span a shell separator: `git status && rm -rf /`
          // starts with the allowed prefix, but the appended command was never vetted, so
          // granting Allow here bypasses the sandbox (node_7cd9e73255b1). Only Forbidden may
          // match a chained command broadly. A prefix rule without a separator is unaffected.
          if (rule.decision === 'Forbidden' || !HAS_COMMAND_SEPARATOR_RE.test(trimmed)) {
            matched = true
          }
        }
