...
export function assertTrustedMcpServer(spec: McpServerSpec, options: AllowlistOptions = {}): void {
  // Match by ROLE (basename), not by the exact invocation path: a resolved absolute path
  // like `/usr/local/bin/node` or process.execPath IS node. Callers reach the same binary
  // through different paths (`node` vs `which node` vs execPath); an exact-string match
  // rejected the healthy full-path case and made `agf mcp probe` report reachable:false
  // (node_bc3f3c78cc29). Basename is not a weaker control here — an attacker who can set an
  // arbitrary command path can already name their binary `node`; the real gate is the
  // role + the arg checks below.
  ...
  if (commandName === 'npx') {
    const parsed = parseNpxCommand('npx', spec.args)
    if (!parsed) {
