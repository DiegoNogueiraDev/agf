...
// Shell separators that start a new command. Split on these before running the
// ^-anchored interpreter-eval rule so a chained eval (`true; node -e …`) can't hide
// behind a benign first command. The pattern rules below use .includes()/unanchored
// regexes, so only INTERPRETER_INLINE_EVAL_RE needs per-segment evaluation.
...
function isDangerous(command: string): boolean {
  ...
  // Interpreter inline-eval is ^-anchored: test each chained segment, not just the first.
  for (const segment of cmd.split(SEPARATOR_RE)) {
    if (INTERPRETER_INLINE_EVAL_RE.test(segment.trim())) return true
  }
  return false
}
