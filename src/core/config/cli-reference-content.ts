...
/**
 * Heading of the universal doctrine block. Exported so consumers (and the guard
 * tests) locate the section without duplicating the literal.
 */
export const UNIVERSAL_RULES_HEADING = '### 🌍 Regras Universais de Engenharia (valem em QUALQUER projeto)'

/**
 * 🌍 Universal engineering doctrine — the project-agnostic FLOOR every agf user
 * inherits, in every CLI and every verbosity mode.
 *
 * WHY this exists separately from {@link AGF_GOLDEN_RULES}: those rules teach an agent
 * how to drive *agf itself* (they name agf commands and only make sense inside a repo
 * that uses it). These are engineering rules that hold in any codebase, in any language,
 * driven by any CLI — so they are deliberately written WITHOUT naming a single command.
 * That is a contract, not a style choice: a guard test asserts the block cites no agf
 * command, because a "universal" rule that presumes a binary on PATH is not universal.
 *
 * The two blocks SUM — this one never replaces the agf-specific set.
 */
/** Uma regra do piso universal e QUEM a cobra. */
export interface UniversalRule {
  /** Enunciado em markdown, já com o **negrito** do título e o porquê. */
  text: string
  /**
   * `advisory`, ou `enforced:<gatilho>` nomeando o hook/gate que a cobra.
   *
   * Nada é `enforced` por default: alegar uma guarda inexistente ensina o leitor
   * a confiar num check que ninguém roda — pior que não ter guarda. Hoje só duas
   * regras têm gatilho determinístico, e essa é justamente a verdade que a regra
   * do enforcement precisa expor sobre si mesma.
   */
  enforcement: string
}

/** As regras do piso, como DADOS — uma regra nova não existe sem decidir quem a cobra. */
export const UNIVERSAL_RULES: readonly UniversalRule[] = [
  {
    text: `**Investigue antes de codar — não duplique.** Leia o estado real (histórico, branches,
  ...
  {
    text: `**Investigue e EXPANDA, nunca recrie.** Antes de criar arquivo/função/módulo, procure
   o dono do comportamento e estenda-o. Criar do zero só quando comprovadamente não existe.`,
    enforcement: 'advisory',
  },
  {
    text: `**Fonte única de verdade (DRY).** Cada conhecimento tem UMA representação autoritativa;
   N cópias = N−1 bugs. Ao ver o mesmo bloco em 2+ lugares, extraia e referencie.`,
    enforcement: 'advisory',
  },
  {
    text: `**Legibilidade é o maior multiplicador.** Escreva para minimizar o tempo de quem vai LER
   ...
   early returns em vez de aninhamento, nomes autodescritivos, zero código morto.`,
    enforcement: 'enforced:file-size-guard',
  },
  {
    text: `**Núcleo puro, I/O nas bordas (SOLID/DIP).** A lógica de domínio depende de portas
   injetáveis, nunca de I/O concreto — adapters em arquivo à parte, núcleo testável isolado.`,
    enforcement: 'advisory',
  },
  {
    text: `**Teste contra a fonte REAL, nunca contra um dublê.** Valide com dados reais ou dados
  ...
  {
    text: `**Pronto só é real se critério ↔ código ↔ teste baterem no disco.** Falta código OU teste
  ...
  {
    text: `**Prove o valor no modo do consumidor real.** Reproduza o comportamento prometido pelo
  ...
  {
    text: `**Valide operando a INTERFACE, não só a API.** Onde há tela, exercite a tela: o backend
  ...
  {
    text: `**Enforcement real é gatilho determinístico, não agente lembrando.** O que precisa valer
  ...
  {
    text: `**Capacidade dormente é vazamento de valor.** Código pronto-mas-não-conectado (exportado
    sem consumidor, sem superfície, atrás de flag desligada) entrega zero — meça, wire e cobre.`,
    enforcement: 'advisory',
  },
  {
    text: `**Gerar menos é a maior alavanca de custo.** Tokens de saída custam várias vezes os de
    entrada: reuse scaffold e edite só a região alterada, em vez de regerar do zero.`,
    enforcement: 'advisory',
  },
  {
    text: `**Filtre na origem antes de ler.** Para achar/contar/cruzar, use a ferramenta
    ...
    enforcement: 'enforced:shell-output-compression',
  },
  {
    text: `**Comente o porquê, nunca o óbvio — e mantenha o comentário vivo.** Registre invariante,
    decisão e gotcha; comentário que mente é pior que ausência. Código vence comentário.`,
    enforcement: 'advisory',
  },
]

/** Renderiza a lista numerada, anexando o marcador de enforcement a cada regra. */
function renderUniversalRules(): string {
  return UNIVERSAL_RULES.map((r, i) => `${i + 1}. ${r.text}\n   \`[${r.enforcement}]\``).join('\n')
}

export const AGF_UNIVERSAL_RULES = `${UNIVERSAL_RULES_HEADING}
