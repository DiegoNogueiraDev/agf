...
import { stampBody } from './boundary-drift.js'
import { filterRulesByStack, type RulePack } from './language-rules-filter.js'
...
/**
 * O que um PROJETO acrescenta ao piso universal.
 *
 * O gerador recebe isto já lido — quem lê a fonte (nós de constitution no grafo,
 * catálogo de packs em disco) é o chamador, e é lá que uma fonte malformada é
 * absorvida. Assim o gerador continua puro e uma constitution quebrada nunca
 * derruba a emissão do piso, que é justamente o conteúdo que não pode faltar.
 */
export interface ProjectDoctrine {
  /** Princípios do projeto, um por item. */
  principles?: readonly string[]
  /** Catálogo de rule-packs; filtrado por {@link languages} antes de render. */
  rulePacks?: readonly RulePack[]
  /** Linguagens ativas do projeto — packs de outras linguagens não entram. */
  languages?: readonly string[]
}

/**
 * Renderiza o que o projeto acrescenta, ou string vazia quando não há nada.
 *
 * Vazio é literalmente vazio: sem cabeçalho órfão, sem placeholder. Um projeto
 * que nada acrescenta produz saída byte-idêntica à de antes desta capacidade —
 * é o que torna a extensão aditiva de fato, e não só na intenção.
 */
function renderProjectDoctrine(doctrine?: ProjectDoctrine): string {
  if (!doctrine) return ''
  const principles = doctrine.principles ?? []
  const packs = filterRulesByStack([...(doctrine.languages ?? [])], [...(doctrine.rulePacks ?? [])])
  if (principles.length === 0 && packs.length === 0) return ''

  const parts = ['### 🏛️ Doutrina deste projeto (soma ao piso universal, nunca o substitui)']
  if (principles.length > 0) {
    parts.push(principles.map((p, i) => `${i + 1}. ${p}`).join('\n'))
  }
...
/** Insert the provenance stamp as the first line inside the markers. */
function stampSection(section: string): string {
  const startIdx = section.indexOf(MARKER_START)
  const endIdx = section.indexOf(MARKER_END)
  if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx) return section
  const bodyStart = startIdx + MARKER_START.length
  const body = section.slice(bodyStart, endIdx).trim()
  return `${section.slice(0, bodyStart)}\n${stampBody(body)}\n${section.slice(endIdx)}`
}
...
${LEAN_DISCOVERY_HINT}${renderProjectDoctrine(doctrine)}`
  }
...
export function generateClaudeMdSection(
  ...
  return `
${MARKER_START}
${buildSectionBody(projectName, mode, projectContext, doctrine)}
${MARKER_END}
`
}
...
export function applySection(rawExistingContent: string, newSection: string): string {
  ...
  // Stamp the marked region with a hash of its own body, so a later drift check can
  // tell "agf generated this, it is just older" from "a human typed in here" as a
  // FACT. Without it the two are the same bytes once canonical changes.
  const trimmedSection = stampSection(newSection.trim()) + '\n'
