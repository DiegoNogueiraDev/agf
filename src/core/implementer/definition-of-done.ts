...
import { checkTddGate } from '../presets/tdd-workflow-gate.js'
import { getActivePresetNameFromDb } from '../presets/preset-gate-adapter.js'
...
import { isSurfaceTask } from './surface-task.js'
...
export interface DodOptions {
  ...
  dir?: string
  /** Confirms a failing test (RED) was observed before the implementation passed (GREEN). */
...
export function checkDefinitionOfDone(doc: GraphDocument, nodeId: string, opts: DodOptions = {}): ImplementDoneReport {
  ...
  // tdd_gate_pass — mandatory red-before-green gate (tdd-workflow-gate.ts).
  // Only blocks when the project's active preset is strict-tdd AND no red
  // test was confirmed; no preset (or any other preset) stays non-blocking.
  const activePreset = opts.db ? getActivePresetNameFromDb(opts.db) : undefined
  const tddGate = checkTddGate({ activePreset, hasRedTestFirst: opts.hasRedTestFirst ?? false })
  checks.push({
    name: 'tdd_gate_pass',
    passed: !tddGate.blocked,
    details: tddGate.blocked ? tddGate.reason : 'TDD gate: ok',
    severity: 'required',
  })
  ...
  // Analyse what the task DECLARES it implements, falling back to the touched set.
  // `touched` is only populated at done-time, so keying off it alone made this check
  // report "no implementation files declared" for nodes that declare them — a message
  // describing a state that was not the reason.
  const complexityInputs = node.implementationFiles?.length ? node.implementationFiles : touched
  const implFiles = complexityInputs.map((p) => (isAbsolute(p) ? p : joinPath(process.cwd(), p)))
  ...
  // Karpathy principle 3 (Surgical Changes): compare the scope the node declares
  // against the files actually touched.
  //
  // The scope MUST be read through declaredFilesOf — the same helper the colony
  // batcher and the scope-creep detector use — because that is where the CLI
  // writes it (top-level implementationFiles/testFiles). This check previously
  // read `metadata.declaredFiles`, a path nothing ever writes, so it skipped on
  // every node while reporting "no declared scope" as the reason. A skip that
  // scores as green is worse than a failure: a node with scope and one without
  // become indistinguishable, and the blast-radius gate rests on this field.
  // NOTE — no on-disk existence check here, deliberately. This function has no
  // project root of its own (it resolves against process.cwd()), so a phantom-file
  // check would fire falsely whenever the DoD is evaluated for a graph rooted
  // elsewhere. The physical half of the triangulation is already enforced where the
  // root IS known: `agf done` refuses a node whose declared files are absent under
  // its --dir. Duplicating it here buys nothing and costs false positives.
  const surgicalResult = evaluateSurgicalScope({ declaredFiles: declaredFilesOf(node), modifiedFiles: touched })
  checks.push({
  ...
  // 14. consumer_proof — rule 16: every delivery must prove value +
  // behavior in the consumer's real mode. Recommended severity:
  // flags tasks with no consumerProof evidence without blocking the gate.
  const rawProof = (node.metadata as Record<string, unknown> | undefined)?.consumerProof
  const consumerProof =
    rawProof && typeof rawProof === 'object' && !Array.isArray(rawProof) ? (rawProof as Record<string, unknown>) : null
  const hasCommand =
    !!consumerProof && typeof consumerProof.command === 'string' && (consumerProof.command as string).length > 0
  const hasEvidence =
    !!consumerProof && typeof consumerProof.evidence === 'string' && (consumerProof.evidence as string).length > 0
  const consumerProofPassed = hasCommand && hasEvidence
  checks.push({
    name: 'consumer_proof',
    passed: consumerProofPassed,
    details: consumerProofPassed
      ? `Consumer-proof evidência registrada: command="${(consumerProof as Record<string, string>).command}"`
      : !consumerProof
        ? 'Nenhum consumerProof registrado — adicione evidência de execução no modo do consumidor real'
        : !hasCommand
          ? 'consumerProof sem command válido — o campo "command" é obrigatório'
          : 'consumerProof sem evidence — adicione o caminho da evidência (screenshot/log)',
    severity: 'recommended',
  })
  ...
  const DOD_FIX_HINTS: Record<string, string> = {
    ...
    tdd_gate_pass: 'Write a failing test first, then re-run `agf check <id> --red-first` to confirm RED-before-GREEN',
    ...
    consumer_proof:
      'Add consumer proof evidence: `agf node update <id> --metadata.consumerProof \'{"command":"...","evidence":"..."}\'`',
  }
  ...
  // Surface self-declaration (node_919687afcea8): recognition only — does NOT
  // add/remove any check, so non-surface gate behavior stays byte-identical.
  const isSurface = isSurfaceTask(doc, nodeId)

  return { nodeId, title: node.title, checks, ready, score, grade, summary, isSurface }
}
