...
const STOPWORDS = new Set([
  ...
  'do',
  'em',
  // 'no' is NOT here. Folding turns `nó` into `no`, and dropping it as a preposition threw away
  // the subject of every question about a node: "mostrar um nó do grafo" became "mostrar grafo",
  // which is how `agf session show` came to answer it. The preposition is noise; the noun is the
  // question. In this corpus the noun wins.
  'na',
  'para',
  'e',
  'que',
...
function tokenize(text: string): string[] {
  ...
  //
  // Fold first, or the split eats the accent and the word with it: `código` became `c` + `digo`,
  // two tokens that mean nothing. Applied to both sides — the corpus is written in Portuguese
  // half the time.
...
/**
 * One group per query concept: the word the person typed, plus what it means in the other
 * language the corpus speaks. Kept as groups rather than a flat list so confidence can ask
 * "was this concept covered?" instead of "were all six spellings present?".
 */
function conceptGroups(query: string): string[][] {
  return contentTokens(query).map((token) => [...expand(token)])
}

/**
 * How much of the question the chunk answers, weighted by how much each concept was worth.
 *
 * A concept counts as covered when any of its spellings appears — "código" is answered by "code".
 * Counting concepts equally let `agf adr create` beat `agf question` for "registrar uma pergunta
 * em aberto no grafo": *Architecture Decision **Record*** matched "registrar" and the description
 * happened to say "grafo", two cheap words against one expensive one. Rarity decides. A concept
 * nobody else mentions is the concept that identifies the command.
 *
 * This is both the ranking sort and the confidence gate, deliberately: an answer is only as good
 * as the share of the question it addresses.
 */
function coverage(
  groups: readonly string[][],
  chunk: CommandChunk,
  weightOf: (concept: readonly string[]) => number,
): number {
  const total = groups.reduce((sum, group) => sum + weightOf(group), 0)
  if (total === 0) return 0
  const tokens = new Set(tokenize(docText(chunk)))
  const covered = groups
    .filter((group) => group.some((term) => tokens.has(term)))
    .reduce((sum, group) => sum + weightOf(group), 0)
  return covered / total
}

/**
 * Inverse document frequency of a concept: the rarest of its spellings, because that is the one
 * that carries the information. Memoised per retrieval — the corpus does not change mid-call.
 */
function conceptWeigher(corpus: readonly CommandChunk[]): (concept: readonly string[]) => number {
  const N = corpus.length
  const df = new Map<string, number>()
  for (const chunk of corpus) {
    for (const term of new Set(tokenize(docText(chunk)))) df.set(term, (df.get(term) ?? 0) + 1)
  }
  ...
  return (concept) => {
    const key = concept.join('|')
    const hit = cache.get(key)
    if (hit !== undefined) return hit
    const weight = Math.max(...concept.map(idf))
...
export function retrieveCommand(
  ...
  const groups = conceptGroups(query)
  // Rank against every spelling of every concept; judge coverage per concept, below.
  const qAll = [...new Set([...tokenize(query), ...groups.flat()])]
  ...
  // Rank and gate must agree. Asked to "trocar a marcha de esforço do modelo", RRF put
  // `agf model set` first — covering two of four concepts — while `agf gearshift`, covering
  // three, never reached the podium. The gate then judged the winner by a metric the ranking
  // had never consulted, and passed it at exactly the threshold. So coverage sorts the pool
  // before the pool is cut, and RRF breaks its ties.
  const weightOf = conceptWeigher(corpus)
  const reranked = rerankCandidates(query, pool, poolSize)
  const candidates = reranked
    .map((candidate, order) => ({ candidate, order, cover: coverage(groups, candidate.chunk, weightOf) }))
    .sort((a, b) => b.cover - a.cover || a.order - b.order)
    .slice(0, k)
    .map((entry) => entry.candidate)

  const top = candidates[0]?.chunk ?? null
  const confidence = top ? coverage(groups, top, weightOf) : 0
