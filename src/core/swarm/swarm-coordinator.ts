...
import type { SwarmConfigInput, Topology, ConsensusKind, AgentRole } from './swarm-types.js'
...
export class SwarmCoordinator {
  ...
  /**
   * Registra um agente na sessão (INSERT em swarm_agents). Idempotente por
   * (session, agentId): a chave de linha é `${sessionId}:${agentId}`, então
   * re-registrar o mesmo agente na mesma sessão é no-op — casa a idempotência
   * do provisionamento de worktree (ant-swarming spawn re-rodado). Lança se a
   * sessão não existir (FK garante integridade, mas o erro tipado é mais claro).
   */
  join(sessionId: string, input: { agentId: string; role?: AgentRole }): void {
    if (!this.getRow(sessionId)) {
      throw new McpGraphError(`Swarm session not found: ${sessionId}`)
    }
    const nowIso = new Date().toISOString()
    this.db
      .prepare(
        `INSERT OR IGNORE INTO swarm_agents (id, session_id, role, status, created_at, updated_at)
         VALUES (?, ?, ?, 'active', ?, ?)`,
      )
      .run(`${sessionId}:${input.agentId}`, sessionId, input.role ?? 'worker', nowIso, nowIso)
    log.info('swarm:join', { sessionId, agentId: input.agentId })
  }

  /** Agentes registrados na sessão. */
  agents(sessionId: string): Array<{ id: string; role: string; status: string }> {
    return this.db
      .prepare('SELECT id, role, status FROM swarm_agents WHERE session_id = ? ORDER BY created_at')
      .all(sessionId) as Array<{ id: string; role: string; status: string }>
  }

  /** Nº de agentes registrados na sessão. */
  agentCount(sessionId: string): number {
    const row = this.db.prepare('SELECT COUNT(*) AS n FROM swarm_agents WHERE session_id = ?').get(sessionId) as {
      n: number
    }
    return row.n
  }
