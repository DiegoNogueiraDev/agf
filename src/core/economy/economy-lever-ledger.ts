...
export function recordLeverEvent(db: Database.Database, event: LeverEvent): string {
  ...
  db.prepare(
    `INSERT INTO economy_lever_ledger
      (id, ts, session_id, node_id, lever, tokens_before, tokens_after, saved, accepted, gate_outcome, score)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    id,
    Date.now(),
...
export interface ScaffoldRecoverySummary {
  ...
  generated: number
  /** SUM(saved) across accepted scaffold_recovery events. */
  tokensSaved: number
...
/**
 * Summarize the `scaffold_recovery` lever specifically: how often a scaffold
 * was reused (accepted) vs generated fresh (passthrough), and the resulting
 * token economy. Composed by proof-snapshot.ts's `scaffoldReuse` field.
 */
