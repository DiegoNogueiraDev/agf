...
export function mergeImportedTau(
  ...
  // Fronteira de confiança: o bundle vem de OUTRO projeto e pode chegar
  // corrompido (arquivo truncado, par com bug, JSON editado à mão). Um valor
  // não-finito não pode ser "clampado" para dentro da faixa — NaN escorreria
  // para o banco (`local >= NaN` é falso) e Infinity viraria τ_max, ou seja,
  // lixo recebendo a confiança MÁXIMA. Recusar é a única leitura honesta de um
  // número que não é número.
  if (!Number.isFinite(imported.amount)) return false
