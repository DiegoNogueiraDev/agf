...
// ── Distorção graduada (E4.T1, node_ed4e8516b4c0) ──

const RE_PATH = /(?:[\w.-]+\/)+[\w.-]+/g

/** Todas as ocorrências das entidades que os verifies booleanos já protegem. */
function extractEntities(text: string): string[] {
  const out: string[] = []
  for (const re of [RE_URL, RE_EMAIL, RE_CODE_FENCE, RE_DATE, RE_TIME, RE_PATH]) {
    out.push(...(text.match(new RegExp(re.source, 'g')) ?? []))
  }
  out.push(...(text.match(new RegExp(RE_NUMBER.source, 'g')) ?? []).filter((n) => n.length >= 3))
  return out
}

/**
 * Distorção ∈ [0,1]: fração de entidades (URLs, e-mails, fences, datas, horas,
 * paths, números ≥3 dígitos) do original PERDIDAS no comprimido — a versão
 * GRADUADA dos verifies booleanos acima (mesma família de sinais, DRY).
 * Retenção multiset: remover metade das instâncias conta metade. Texto sem
 * entidades → 0 (ponto cego declarado da proxy; a rubrica LLM audita por
 * amostragem — risk node_126fa2428a58). Alimenta a varredura RD (E4.T2) e o
 * gate de regressão de distorção (E4.T3).
 */
export function measureDistortion(original: string, compressed: string): number {
  if (original === compressed) return 0
  if (compressed.trim().length === 0 && original.trim().length > 0) return 1

  const originalEntities = extractEntities(original)
  if (originalEntities.length === 0) return 0

  const available = new Map<string, number>()
  for (const e of extractEntities(compressed)) {
    available.set(e, (available.get(e) ?? 0) + 1)
  }
  let retained = 0
  for (const e of originalEntities) {
    const left = available.get(e) ?? 0
    if (left > 0) {
      retained += 1
      available.set(e, left - 1)
    }
  }
  return 1 - retained / originalEntities.length
}
