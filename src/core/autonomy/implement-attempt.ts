 ...
 */
/**
 * Normaliza uma mensagem de falha para comparar attempt-a-attempt (node_aa95187d6e16):
 * trim + colapsa espaços + lowercase. Duas falhas que só diferem em espaçamento/caixa
 * contam como a MESMA — retry idêntico não faz progresso. Determinística, ~0 token.
 */
function normalizeFailure(raw: string): string {
  return raw.trim().replace(/\s+/g, ' ').toLowerCase()
}

/** Ordena os níveis de reasoning-effort para detectar "não subiu mais" (escada esgotada). */
const EFFORT_RANK: Record<ReasoningEffort, number> = { minimal: 0, low: 1, medium: 2, high: 3 }
function effortRank(e: ReasoningEffort): number {
  return EFFORT_RANK[e] ?? 0
}
...
export async function attemptImplementation(deps: AttemptDeps, options: AttemptOptions): Promise<AttemptOutcome> {
  ...
  // node_aa95187d6e16 — assinatura + effort da falha anterior. Só aborta quando a
  // falha se repete IDÊNTICA E o effort já não sobe mais (escada de raciocínio
  // esgotada): re-tentar com mesmo erro e mesmo effort não faz progresso.
  let prevFailureSig: string | undefined
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    // ── No-progress guard (node_aa95187d6e16) ─────────
    // O effort escala com o attempt (low→medium→high): cada vermelho é uma
    // tentativa GENUINAMENTE mais forte, não repetição. Só freamos quando a
    // falha é idêntica À anterior E o effort já não sobe (escada esgotada).
    ...
    if (attempt >= 2) {
      const curSig = normalizeFailure(lastError ?? '')
      const escalated = prevEffort !== undefined && effortRank(effort) > effortRank(prevEffort)
      if (curSig !== '' && prevFailureSig === curSig && !escalated) {
        log.warn('implement-attempt:no-progress — falha idêntica + effort no teto, abortando retry', {
          attempt,
    ...
    // effort já computado no topo do loop (no-progress guard) — reusa.
    // 1) Provider call
    let text: string
    try {
