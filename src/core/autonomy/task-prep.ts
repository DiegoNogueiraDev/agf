...
import { forageCutIsSafe } from './forage-cut-guard.js'
...
export interface PrepareTaskOptions {
  ...
  /**
   * Braço de controle: quando dado, ESTA é a config de levers da execução —
   * nem o persistido nem o bundle auto-ativado entram.
   *
   * Existe para tornar um lever mensurável sem escrever na config do projeto de
   * quem roda o experimento. Ausente ⇒ byte-idêntico (ver
   * {@link resolveLeversForRun}).
   */
  leversOverride?: EconomyLeversConfig
}
...
export interface TaskPreparation {
  ...
  /**
   * True quando a rede desfez o corte do `forage_stop` (node_243c93c7c8a2).
   *
   * Exposto de propósito: uma rede de segurança que age em silêncio não pode ser
   * medida nem auditada, e ninguém descobre que ela está disparando toda hora —
   * ou nunca.
   */
  forageReverted: boolean
}
 ...
 */
/**
 * Levers efetivos DESTA execução, com braço de controle opcional.
 *
 * PORQUÊ: {@link resolveEffectiveLevers} liga o bundle loss-safe
 * INCONDICIONALMENTE quando um agente dirige (node_7665ce2ed19d). Sem um jeito
 * de neutralizar isso por chamada, os dois braços de qualquer A/B recebem os 5
 * levers ligados e a medição dá zero por CONFUNDIMENTO — um resultado que se lê
 * como "o lever não serve" e na verdade significa "o experimento nunca teve
 * grupo de controle".
 *
 * `override` ausente ⇒ byte-idêntico ao comportamento de hoje, então nenhum
 * caminho existente muda. `override` presente ⇒ é a config, ponto: nem o
 * persistido nem o bundle entram, porque um braço que herda levers vizinhos
 * torna impossível atribuir o ganho ao lever em teste.
 *
 * PURA: lê a fonte e devolve um valor. Nunca persiste — um experimento não pode
 * deixar o projeto de quem o rodou diferente de como estava.
 */
export function resolveLeversForRun(
  source: EconomyLeversConfigSource,
  override?: EconomyLeversConfig,
): EconomyLeversConfig {
  return override ?? resolveEffectiveLevers(source)
}
...
export async function prepareTask(
  ...
  const leversForRepo = resolveLeversForRun(store, opts.leversOverride)
  ...
  const repoMapResult =
    repoSymbols && repoSymbols.length > 0
      ...
      : undefined
  // ── Rede do corte agressivo (node_243c93c7c8a2) ────────────────────────────
  // `forage_stop` corta 62–96% do repo-map e fica LIGADO por default quando um
  // agente dirige. Se o corte removeu todo símbolo relacionado à task, ele
  // cortou exatamente aquilo por que o mapa existe — então reconstrói-se o mapa
  // SEM forage e usa-se ele. O guard só roda quando o lever está ligado: com ele
  // desligado o caminho é byte-idêntico e não paga nada.
  let repoMap = repoMapResult?.text || undefined
  let forageReverted = false
  if (repoMap && repoSymbols && repoSymbols.length > 0 && isLeverEnabled(leversForRepo, 'forage_stop')) {
    const semForage = buildRepoMap(
      { symbols: repoSymbols, relations: repoRelations ?? [] },
      {
        tokenBudget: REPO_MAP_TOKEN_BUDGET,
        focus: node.title,
    ...
    if (semForage && !forageCutIsSafe(semForage, repoMap, node.title)) {
      repoMap = semForage
      forageReverted = true
      opts.onLog?.('forage_stop: corte revertido — removia todo símbolo relacionado à task')
    }
  }
  ...
  if (opts.projectDir) {
    try {
      ...
      const leversCfg = resolveLeversForRun(store, opts.leversOverride)
  ...
  return { repoMap, flowContext, reuse, signature, priorMemories, pheromoneTrails, forageReverted }
}
