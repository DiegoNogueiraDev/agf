...
export function checkBoundaryDriftWith(
  ...
  // Two causes, two very different messages. Calling an upgrade-induced mismatch
  // "hand-edited" accuses every already-initialized project of tampering the moment
  // the generator grows a section — a false alarm that teaches users to ignore this
  // check. See DriftCause in boundary-drift.ts for the discriminator and its limit.
  if (findings[0].cause === 'indeterminate') {
    return {
      name: 'boundary-drift',
      level: 'warning',
      message: `${fileName} differs from the generated section and predates provenance stamping — cannot tell an old version from a hand-edit`,
      suggestion: `Regenerate the marked section in ${fileName}; it will then be stamped and future checks are exact`,
    }
  }

  if (findings[0].cause === 'outdated') {
    return {
      name: 'boundary-drift',
      level: 'warning',
      message: `${fileName} is out of date — the generated section grew since it was written (no hand-edits found)`,
      suggestion: `Regenerate the marked section in ${fileName} to pick up the new content`,
    }
  }
