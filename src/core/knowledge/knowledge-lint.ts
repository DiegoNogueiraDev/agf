 ...
 * runDialecticEngine), under_linked_entity (term in ≥3 facts with zero edges,
 * via detectUnderLinkedEntities). Zero deletions — callers decide what to act on.
 *
...
import { detectUnderLinkedEntities } from '../memory/ingest-crosslink.js'
...
export function lintKnowledge(db: Database): KnowledgeLintResult {
  ...
  if (tableExists(db, 'memory_facts')) {
    ...
    // 3b. under_linked_entity — term referenced across ≥3 facts but never cross-linked
    if (hasEdges) {
      const { underLinkedEntities } = detectUnderLinkedEntities(db)
      for (const entity of underLinkedEntities) {
        findings.push({ id: entity, reason: 'under_linked_entity' })
      }
    }
  }
