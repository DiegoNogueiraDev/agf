...
/** True if the task has at least one `tests` edge (a test node verifying it). */
function hasTestEdge(doc: GraphDocument, taskId: string): boolean {
  return doc.edges.some((e) => e.relationType === 'tests' && (e.from === taskId || e.to === taskId))
}
...
export interface FullChainEntry {
  ...
  linkedTasks: string[]
  /** Of those, the ones that have a `tests` edge. */
  testedTasks: string[]
 ...
 */
export function buildFullChainTraceability(doc: GraphDocument): FullChainReport {
  const requirements = doc.nodes.filter((n) => n.type === 'requirement')
  const entries: FullChainEntry[] = requirements.map((req) => {
    const linkedTasks = findLinked(doc, req.id, TASK_TYPES)
    const testedTasks = linkedTasks.filter((t) => hasTestEdge(doc, t))
