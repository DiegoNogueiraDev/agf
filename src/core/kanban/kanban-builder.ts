...
function filterNodes(nodes: GraphNode[], config: KanbanConfig): GraphNode[] {
  let result = config.showOnlyTasks ? nodes.filter((n) => n.type === 'task' || n.type === 'subtask') : [...nodes]
  if (config.sprintFilter) {
    result = result.filter((n) => n.sprint === config.sprintFilter)
  }
  return result
}
