...
import { findNextTask, type NextTaskOptions } from './next-task.js'
...
const TASK_RESOURCE_PREFIX = 'task'
...
/**
 * REQ-LCR-003 (docs/prd/graph-leaf-cutter.md): the Petri-net token model — WIP>1
 * across agents is only safe when their tasks touch disjoint files. Scans every
 * OTHER agent's active lease, looks up the leased node in `doc`, and unions its
 * declared `metadata.touchedFiles`. The claiming agent's own lease is excluded so
 * re-claiming/renewing its current task is never self-blocked.
 */
function collectOtherAgentsTouchedFiles(doc: GraphDocument, locks: LockManager, agentId: string): Set<string> {
  ...
  for (const lock of locks.listActive()) {
    if (lock.agentId === agentId || !lock.resourceId.startsWith(`${TASK_RESOURCE_PREFIX}:`)) continue
    const nodeId = lock.resourceId.slice(TASK_RESOURCE_PREFIX.length + 1)
    const node = doc.nodes.find((n) => n.id === nodeId)
    const touched = (node?.metadata as Record<string, unknown> | undefined)?.touchedFiles
    if (Array.isArray(touched)) {
      for (const f of touched) if (typeof f === 'string') files.add(f)
    }
  }
  return files
}
...
export function claimNextTask(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      const lease = locks.acquire(`${TASK_RESOURCE_PREFIX}:${result.node.id}`, agentId, opts.ttlSeconds)
...
export function findAgentClaim(db: Database.Database, agentId: string): string | null {
  ...
  return row.resource_id.startsWith('task:') ? row.resource_id.slice(5) : row.resource_id
}
