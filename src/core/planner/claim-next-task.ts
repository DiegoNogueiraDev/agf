...
export function claimNextTask(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    // node_22361e1331e3: `findNextTask` sinaliza `all_tasks_blocked` devolvendo a task
    // MENOS-bloqueada (mas ainda com depends_on pendente) — um sinal p/ consumidores que
    // só EXIBEM. O claim NUNCA pode reivindicar/entregar uma task bloqueada: nada é
    // claimable quando tudo tem dep pendente. next-cmd já mapeia null → NO_TASKS
    // ('all_claimed_or_blocked').
    if (result.warning === 'all_tasks_blocked') return null
