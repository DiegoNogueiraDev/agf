...
import { findNextTask, findUnblockedTasks, type NextTaskResult } from './next-task.js'
...
export function selectNextTaskSmart(doc: GraphDocument, params: SelectParams): NextTaskResult | null {
  const { getDb, getProjectId, mode, rng, pierceContainers, alpha, levy } = params

  if (mode !== 'off') {
    const candidates = findUnblockedTasks(doc)
    if (candidates.length > 0) {
      ...
      if (picked === null) picked = findNextTask(doc, { pierceContainers }) // cold/degenerate field
      ...
      if (picked !== null && withPheromone.some((c) => c.id === picked!.node.id)) {
        try {
          if (acoChosenId !== undefined) {
            const baselineId = findNextTask(doc, { pierceContainers })?.node.id
            if (baselineId) {
  ...
  return findNextTask(doc, { pierceContainers })
}
