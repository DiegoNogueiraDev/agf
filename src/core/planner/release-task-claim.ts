...
export function releaseTaskClaim(
  ...
  // node_884a33abee66 — o claim real (claimNextTask) adquire com o prefixo
  // `task:`; consultar só o id puro tornava o release um no-op silencioso.
  // Cobre os dois formatos (prefixado = atual; puro = locks legados).
  const row = db
    .prepare('SELECT lease_token, agent_id FROM resource_locks WHERE resource_id IN (?, ?)')
    .get(taskResourceId(taskId), taskId) as LockRow | undefined
