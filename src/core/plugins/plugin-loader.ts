...
import type { PluginToolRegistry, PluginToolRegistration } from './plugin-tool-registry.js'
  ...
  return {
    registerTool: (name: string, handler: unknown) => {
      toolRegistry?.register({
        toolName: name,
        pluginName,
        handler: handler as PluginToolRegistration['handler'],
      })
      log.debug(`Plugin registered tool: ${name}`)
    },
...
export class PluginLoader {
  ...
  async loadPlugin(
    ...
    const context = createPluginContext(manifest.name, registryBuilder, toolRegistry)
