...
import { verifyRecovery, type BuildChecker } from './recovery-verification.js'
...
export interface ExecuteOptions {
  dryRun: boolean
  /** Project directory, passed to the post-apply build check (AC1 of recovery-verification). */
  dir?: string
  /** Injectable build checker for testing; defaults to `npx tsc --noEmit`. */
  buildChecker?: BuildChecker
  /** Force the recovery build check even for action types not yet classified as code actions. */
...
export function executeActions(actions: HealingAction[], doc: GraphDocument, options: ExecuteOptions): HealingResult[] {
  ...
  for (const action of actions) {
    ...
    try {
      ...
      const verification = verifyRecovery(action, options.dir ?? '.', {
        buildChecker: options.buildChecker,
        forceCodeAction: options.forceCodeAction,
      })
      ...
      if (verification.outcome === 'failed') {
        results.push({
          ...
          issueId: action.issueId,
          success: false,
          message: `Applied but build check failed: ${verification.errorMessage ?? 'unknown error'}`,
          appliedAt: timestamp,
        })
        continue
      }
