...
function defaultBuildChecker(dir: string): BuildCheckResult {
  const spawnSync = await_import_spawnSync()
...
/** Lazy sync import shim — avoids top-level require in ESM. */
function await_import_spawnSync(): (typeof import('node:child_process'))['spawnSync'] {
  // Node built-in; safe to require synchronously
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  return (require('node:child_process') as typeof import('node:child_process')).spawnSync
}
