...
export interface DoDReport {
  ...
  epicPromotion?: {
    ...
    /** True when a child has ≥1 required completeness gap (checkEpicPromotionGate) or harness < threshold. */
    blocked?: boolean
    requiredGapCount?: number
    /** Last harness score read for the promotion gate; null on cold start. */
    harnessScore?: number | null
    /** Typed harness verdict: HARNESS_BELOW_PROMOTION (blocks) | HARNESS_UNKNOWN (warns). */
    harnessCode?: 'HARNESS_BELOW_PROMOTION' | 'HARNESS_UNKNOWN'
  }
}
