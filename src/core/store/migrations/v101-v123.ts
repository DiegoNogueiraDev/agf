...
export const migrationsV101_V123: Migration[] = [
  ...
  },
  {
    version: 130,
    description:
      'scenario_verdict — veredito de cenário persistido por nodeId, para o gate de surface-proof ler (node_a0e28320fe6b)',
    sql: `
      CREATE TABLE IF NOT EXISTS scenario_verdict (
        id          TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_scenario_verdict_node
        ON scenario_verdict(node_id, ran_at DESC);
    `,
  },
  {
    version: 131,
    description:
      'colony_benchmark_run — wall-clock serial vs batch persistido para o throughput medido aparecer em agf insights (node_414bf84a63d8)',
    sql: `
      CREATE TABLE IF NOT EXISTS colony_benchmark_run (
        id         TEXT PRIMARY KEY,
        project_id TEXT NOT NULL,
        tasks      INTEGER NOT NULL,
        k          INTEGER NOT NULL,
        serial_ms  INTEGER NOT NULL,
        batch_ms   INTEGER NOT NULL,
        speedup    REAL NOT NULL,
        ran_at     INTEGER NOT NULL
      );
      CREATE INDEX IF NOT EXISTS idx_colony_benchmark_project
        ON colony_benchmark_run(project_id, ran_at DESC);
    `,
  },
]
