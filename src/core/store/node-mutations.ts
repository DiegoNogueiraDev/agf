...
export class NodeOps {
  ...
  updateNodeStatus(id: string, status: NodeStatus, options?: MutationOptions): GraphNode | null {
    ...
    // node_8af0904e42fa + node_0c28154d4517 — o dono durável é o PAR
    // (in_progress + claimedBy). Qualquer transição que NÃO entra em
    // in_progress limpa o dono: cobre sair de in_progress (dono morto não
    // assombra o re-open) E o claimedBy de pull sobre task que nunca voou
    // (next --agent grava no claim; se a task é re-roteada sem entrar em
    // in_progress, o dono é órfão). Quem entra em in_progress regrava o dono
    // na sequência (next/node status). Sem claimedBy ⇒ byte-idêntico.
    const oldMetadata = oldNode.metadata as Record<string, unknown> | undefined
    if (status !== 'in_progress' && oldMetadata?.claimedBy !== undefined) {
      const { claimedBy: _dropped, ...rest } = oldMetadata
      setClauses.push('metadata = ?')
      params.push(Object.keys(rest).length > 0 ? JSON.stringify(rest) : null)
    }
