...
export async function runScenario(
  ...
  try {
    ...
    // runBuildOrchestration lança em escalação — conta como não-resolvido.
    deps.onLog?.(`escalou: ${err instanceof Error ? err.message : String(err)}`)
  }
 ...
 * `prd` inline ou `prdFile` relativo. Filtro opcional por tier.
 */
export function loadSuite(suiteDir: string, tierFilter?: string): Scenario[] {
