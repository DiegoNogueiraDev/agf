...
import { computeIndexHonesty as computeIndexHonestyRatio } from './index-honesty.js'
 ...
 * Delegates the threshold math to index-honesty.ts (single source of truth).
 */
export function computeIndexHonesty(input: IndexHonestyInput): IndexHonestyResult {
  return { bruteForce: computeIndexHonestyRatio(input.selected, input.corpusSize).bruteForce }
}
