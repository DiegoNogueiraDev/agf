...
import { readFile as readRolloutFile } from './rollout-compression.js'
...
export class LocalThreadStore implements ThreadStore {
  ...
  async loadHistory(params: LoadThreadHistoryParams): Promise<RolloutEntry[]> {
    ...
    const readablePath = existsSync(path) ? path : existsSync(gzPath) ? gzPath : null
    ...
    try {
      const raw = readRolloutFile(readablePath)
