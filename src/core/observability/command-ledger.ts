...
export interface CommandLedgerSummary {
  ...
  maxGraphExportBytes: number // MAX(graph_export_bytes) — one full graph read (≈ current graph size)
  activeDays: number // COUNT(DISTINCT calendar day) — read-episode count for bounding the baseline
}
...
export function summarizeCommandLedger(db: Database.Database, opts: { sessionId?: string } = {}): CommandLedgerSummary {
  ...
  const row = db
    .prepare(
      `SELECT COUNT(*) AS calls,
              ...
              COALESCE(MAX(graph_export_bytes), 0) AS max_gexport,
              COUNT(DISTINCT date(ts / 1000, 'unixepoch')) AS active_days
  ...
  return {
    ...
    maxGraphExportBytes: row.max_gexport,
