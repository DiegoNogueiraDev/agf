...
export interface ModelCallRow {
  ...
  /** Formiga que gastou o token (colônia async — node_aa91e9665ac2). Null = loop principal. */
  agentId?: string
}
  ...
  db.prepare(
    `INSERT INTO llm_call_ledger
      (id, ts, project_id, run_id, node_id, caller, provider, model,
       ...
       model_tier, escalated, escalation_reason, agent_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    id,
    Date.now(),
...
/**
 * The driving agent's self-reported cost, kept SEPARATE from agf's own ledger view.
 * `agf submit --tokens` and the OTEL/cost-tracker ingest both write rows with
 * `provider = 'delegated'` (the self-reported marker); this sums ONLY those, so `agf metrics`
 * can show "the conductor spent N tokens on this task" distinct from agf's own calls (0 in
 * delegate mode). Fail-safe: no rows / missing table → all zeros, empty arrays.
 */
export interface ConductorCostSummary {
  source: 'self_reported'
  calls: number
  tokensIn: number
  tokensOut: number
  byNode: Array<{ nodeId: string; tokensIn: number; tokensOut: number }>
  bySession: Array<{ sessionId: string; tokensIn: number; tokensOut: number }>
}

export function summarizeConductorCost(db: Database.Database): ConductorCostSummary {
  const empty: ConductorCostSummary = {
    source: 'self_reported',
    calls: 0,
    tokensIn: 0,
    tokensOut: 0,
  ...
  try {
    const totals = db
      .prepare(
        `SELECT COUNT(*) AS calls, COALESCE(SUM(input_tokens), 0) AS tin, COALESCE(SUM(output_tokens), 0) AS tout
         FROM llm_call_ledger WHERE provider = 'delegated'`,
      )
      .get() as { calls: number; tin: number; tout: number }
    const byNode = db
      .prepare(
        `SELECT node_id AS nodeId, COALESCE(SUM(input_tokens), 0) AS tokensIn, COALESCE(SUM(output_tokens), 0) AS tokensOut
         FROM llm_call_ledger WHERE provider = 'delegated' AND node_id IS NOT NULL
         GROUP BY node_id ORDER BY (SUM(input_tokens) + SUM(output_tokens)) DESC`,
      )
      .all() as Array<{ nodeId: string; tokensIn: number; tokensOut: number }>
    const bySession = db
      .prepare(
        `SELECT session_id AS sessionId, COALESCE(SUM(input_tokens), 0) AS tokensIn, COALESCE(SUM(output_tokens), 0) AS tokensOut
         FROM llm_call_ledger WHERE provider = 'delegated'
         GROUP BY session_id ORDER BY (SUM(input_tokens) + SUM(output_tokens)) DESC`,
      )
      .all() as Array<{ sessionId: string; tokensIn: number; tokensOut: number }>
    return {
      source: 'self_reported',
      calls: totals.calls,
      tokensIn: totals.tin,
...
/**
 * Linha auditável de uma escalada da cascata (A.T2): caller `tier_escalation`,
 * escalated=1 e o motivo do verificador — a marca que o rescue-rate (A.T3) lê.
 */
export function recordTierEscalation(
  db: Database.Database,
  opts: { sessionId: string; nodeId?: string; from: string; to: string; reason: string; costUsd?: number },
): string {
  return recordModelCall(db, {
    sessionId: opts.sessionId,
    nodeId: opts.nodeId,
    caller: 'tier_escalation',
    provider: 'cascade',
    model: opts.to,
    inputTokens: 0,
    outputTokens: 0,
