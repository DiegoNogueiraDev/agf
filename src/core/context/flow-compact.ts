...
/**
 * Bloco `data.flow` do envelope de contexto que o DRIVER lê (contract
 * node_f1e1433c4ddf): valores PROPAGADOS da mesma computação gravada em
 * flow_metrics — nunca recomputados no comando. `prunedTokens` = baseline − actual.
 */
export interface FlowEnvelopeBlock {
  phi: number
  lambda: number
  prunedTokens: number
  pinnedInvariants: number
  arm: FlowMode
}

/** Mapeia o resultado do pipeline para o bloco de envelope (puro, só projeção). */
export function toFlowEnvelopeBlock(result: FlowCompactResult): FlowEnvelopeBlock {
  return {
    phi: result.flow.phi,
    lambda: result.flow.lambda,
    prunedTokens: result.flow.tokensSaved,
    pinnedInvariants: result.flow.pinnedCount,
    arm: result.flow.mode,
  }
}
...
export function applyFlowToCompact(store: SqliteStore, nodeId: string): FlowCompactResult | null {
  ...
  const decayed = buildDecayedTaskContext(store, nodeId, {
    ...
    maxPinnedPullIn: cfg.maxPinnedPullIn,
  })
