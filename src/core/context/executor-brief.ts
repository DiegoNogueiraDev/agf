...
import { decideScaffold, type RagOutDecision, type ScaffoldDescriptor } from '../rag-out/gate.js'
import { loadDefaultScaffoldCorpus } from '../rag-out/scaffold-corpus.js'
import { estimateRagOutEconomy, toLeverEvent } from '../rag-out/economy.js'
...
export interface ExecutorBrief {
  ...
  repoMap?: string
  /**
   * Testes afetados derivados dos implementationFiles do node (F3.T2, TDAD):
   * src/tests/<stem>.test.ts. Presente só quando o node declara arquivos-fonte.
   */
  ...
  reuseHint?: string
  /**
   * Optional output-economy directive: when a rag-out scaffold/mirror matches this task, tell the
   * executor to diff-edit + reuse the scaffold instead of regenerating the whole file. Output costs
   * ~3-5× input, so this is the highest-leverage lever on the conductor's spend. Absent for
   * green-field tasks (no mirror) so a diff-edit is never suggested where there is nothing to diff.
   */
  economyDirective?: EconomyDirective
  /**
 ...
 */
/**
 * Deriva os testes afetados dos arquivos-fonte via convenção do projeto
 * (src/tests/<stem>.test.ts). Puro/determinístico: ignora arquivos que já são
 * *.test.ts (evita self-reference), dedup + ordenação estável. TDAD (F3.T2).
 */
export function deriveAffectedTests(implementationFiles: readonly string[] | undefined): string[] {
  if (!implementationFiles || implementationFiles.length === 0) return []
  ...
  for (const file of implementationFiles) {
    ...
    tests.add(`src/tests/${stem}.test.ts`)
  }
  return [...tests].sort()
}
...
export function buildExecutorBrief(store: SqliteStore, nodeId: string): ExecutorBrief | null {
  ...
  return {
    intent,
    task: taskSummary,
    imitate: (node?.metadata?.imitate as string) ?? PLACEHOLDER_IMITATE,
    readTouch: (node?.metadata?.readTouch as string) ?? PLACEHOLDER_READ_TOUCH,
    contract: (node?.metadata?.contract as string) ?? PLACEHOLDER_CONTRACT,
    acceptanceCriteria,
    notList: [...NOT_LIST],
    blastRadius: bestEffortBlastRadius(store, sourceRef?.file),
    budget: budgetFromXpSize(task.xpSize),
    uncertainty: UNCERTAINTY,
    testWith: (node?.metadata?.testWith as string) ?? PLACEHOLDER_TEST_WITH,
    dod: [...DOD],
    ...
    ...(node?.implementationFiles && node.implementationFiles.length > 0
      ? { affectedTests: deriveAffectedTests(node.implementationFiles) }
      : {}),
  }
}
 ...
 */
/** Tell the executor to diff-edit against a known scaffold instead of regenerating from scratch. */
export interface EconomyDirective {
  mode: 'diff-edit'
  scaffoldPath: string
  instruction: string
}
...
/**
 * Turn a rag-out gate decision into an output-economy directive. Only a `recover` decision (the
 * gate found a scaffold worth reusing) yields one; a `generate` decision (green-field) yields
 * `undefined`, so a diff-edit is never suggested where there is nothing to diff against. Pure.
 */
export function economyDirectiveFor(decision: RagOutDecision): EconomyDirective | undefined {
  if (decision.decision !== 'recover' || !decision.best) return undefined
  const scaffoldPath = decision.best.structureRef ?? decision.best.id
  return {
    mode: 'diff-edit',
    scaffoldPath,
    instruction: `emit diff-edits, reuse scaffold ${scaffoldPath}, não regenere o arquivo inteiro`,
  }
}
...
export async function buildEnrichedBrief(
  store: SqliteStore,
  nodeId: string,
  opts: { projectDir?: string; scaffoldCorpus?: readonly ScaffoldDescriptor[] } = {},
  ...
  // Output-economy directive (E3): when a rag-out scaffold matches, tell the executor to diff-edit
  // instead of regenerating. Absent for green-field tasks. Corpus is injectable for deterministic tests.
  const scaffoldDecision = decideScaffold(brief.task.title, opts.scaffoldCorpus ?? loadDefaultScaffoldCorpus(), {})
  const directive = economyDirectiveFor(scaffoldDecision)
  if (directive) {
    brief.economyDirective = directive
    // Record the scaffold reuse so EPIC 3's value is MEASURABLE (rule 16), not just prose in the
    // brief: recommending a diff-edit against a known scaffold is a rag_out_recovery. The estimate
    // is honestly labelled `structural` by estimateRagOutEconomy (no template body is loaded here).
    // Wrapped — the ledger must never break the brief.
    try {
      recordLeverEvent(store.getDb(), toLeverEvent(estimateRagOutEconomy(scaffoldDecision), `brief_${nodeId}`, nodeId))
    } catch {
      // telemetry stays quiet on a locked db / missing table
    }
  }
...
export function renderBriefMarkdown(brief: ExecutorBrief): string {
  ...
  const lines = [
    ...
    '',
    ...(brief.affectedTests && brief.affectedTests.length > 0
      ? [`**Testes afetados (verificar antes de retornar):** ${brief.affectedTests.join(', ')}`, '']
      : []),
    '**DoD:**',
    bullets(brief.dod),
    '',
  ...
  if (brief.economyDirective) lines.push('', `**Economia (output):** ${brief.economyDirective.instruction}`)
