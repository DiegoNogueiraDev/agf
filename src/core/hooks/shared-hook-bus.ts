...
import { registerSelfHealingListener } from '../skills/self-healing-listener.js'
...
export function getSharedHookBus(): HookBus {
  ...
  if (!registered) {
    ...
    // self-healing-listener.ts: turns `error:detected` graph events into
    // deduplicated healing memories under workflow-graph/memories, consumed
    // by /graph-heal learn. No-op when a custom bus was injected via
    // setSharedHookBus (graphBus is only tracked for the module-owned bus).
    if (graphBus) {
      registerSelfHealingListener({
        memoriesDir: join(process.cwd(), 'workflow-graph', 'memories'),
        eventBus: graphBus,
      })
    }
