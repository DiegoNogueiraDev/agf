...
// ── OKR ───────────────────────────────────────────────────────────────────────
// Mirror of the backend OkrRow/OkrCollection contract (src/core/okr/okr-report.ts).

/** One epic's key result as GET /api/v1/okr serves it. */
export interface OkrRow {
  ...
  current: number | null
  unit: string | null
  /** current/target, or null when no structured KR exists — never 0 for absence. */
...
export interface EconomySnapshot {
  ...
  /**
   * Estado ATIVO de cada lever + a origem do "ligado".
   *
   * Espelha `LeverStateView` do core (`core/web/economy-snapshot.ts`). Esta
   * cópia existe porque o bundle do dashboard não importa do core; mantê-la em
   * sincronia é obrigatório — um campo que o servidor envia e o tipo não declara
   * fica invisível para a UI, que foi como cinco levers ativos ficaram sem
   * aparecer aqui (node_f9978e124d06).
   */
  leverStates: LeverStateRow[]
}

/** Um lever ativo e por que está ligado. Espelha `LeverStateView` do core. */
export interface LeverStateRow {
  name: string
  enabled: boolean
