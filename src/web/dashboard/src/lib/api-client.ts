...
export const apiClient = {
  ...
  /** OKR report (one row per epic) for the OKR tab — same collector as `agf okr`. */
  getOkr: () => request<OkrReport>('/okr'),

  /** Delivery Certainty verdict for one node (Certainty tab). */
  getCertainty: (nodeId: string) => request<DeliveryCertaintyPayload>(`/certainty/${encodeURIComponent(nodeId)}`),
}
