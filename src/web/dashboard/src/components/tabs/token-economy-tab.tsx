...
export function TokenEconomyTab(): React.JSX.Element {
  ...
  // Count EVERY surface the panels below actually render — not just totals/delegate.
  // A NON_TOKEN_LEVER like scaffold_recovery is excluded from totals.saved
  // (savings-tracker.ts), so a delegate-first project whose only economy is lever
  // or scaffold reuse has totals.saved=0; omitting levers/scaffoldReuse/byCommand
  // here rendered "No data" while real savings sat in the sections below.
  const hasActivity =
    totals.saved > 0 ||
    totals.tokensIn > 0 ||
    totals.tokensOut > 0 ||
    ...
    cache.tokensSaved > 0 ||
    commands.calls > 0 ||
    levers.some((lever) => lever.totalSaved > 0) ||
    scaffoldReuse.recovered > 0 ||
    scaffoldReuse.generated > 0 ||
    byCommand.length > 0
  ...
  return (
    <section aria-label="Economy" className="flex flex-col gap-6 p-6 overflow-y-auto h-full">
      ...
      <ActiveLevers states={data?.leverStates ?? []} />
