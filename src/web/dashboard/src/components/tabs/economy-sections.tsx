 ...
 * Levers ATIVOS agora.
 *
 * A tab exibia só quanto cada lever poupou, então cinco levers auto-ativados
 * pelo bundle rodavam invisíveis aqui (node_f9978e124d06). Mostrar a ORIGEM é o
 * que torna a informação acionável: quem ligou à mão pode desligar; quem
 * recebeu do bundle sequer sabia que estava ligado.
 */
export function ActiveLevers({ states }: { states: LeverStateRow[] }): React.JSX.Element {
  const ativos = states.filter((s) => s.enabled)
  return (
    <section aria-label="Active levers" className="flex flex-col gap-3">
      <h2 className="text-sm font-medium text-foreground">Active Levers</h2>
      {ativos.length === 0 ? (
        // Estado-vazio HONESTO: diz que nenhum está ligado, em vez de sumir com
        // a seção e deixar o leitor concluir que a capacidade não existe.
        <p className="text-xs text-muted">No lever is currently enabled.</p>
      ) : (
        <ul className="flex flex-col gap-1.5">
          {ativos.map((s) => (
            <li key={s.name} className="flex items-center gap-2 text-xs">
              <span className="w-32 shrink-0 truncate font-mono text-foreground" title={s.name}>
                {s.name}
              </span>
              <span
                className={
                  s.source === 'auto-bundle'
                    ? 'rounded bg-amber-500/15 px-1.5 py-0.5 text-[10px] text-amber-600'
                    : 'rounded bg-emerald-500/15 px-1.5 py-0.5 text-[10px] text-emerald-600'
                }
                title={
                  s.source === 'auto-bundle'
                    ? 'Ligado automaticamente pelo bundle loss-safe (agente dirigindo). AGF_ECONOMY_AUTO=0 desliga.'
                    : 'Ligado explicitamente na config do projeto.'
                }
              >
                {s.source === 'auto-bundle' ? 'auto-bundle' : 'config'}
              </span>
            </li>
          ))}
        </ul>
