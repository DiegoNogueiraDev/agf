...
describe('nav-config', () => {
  ...
  it('NAV_ITEMS contains exactly the graph + colony + economy tabs', () => {
    const ids = mod.NAV_ITEMS.map((item) => item.id).sort()
    expect(ids).toEqual(['colony', 'economy', 'graph'])
  })
  ...
  it('groups every item under the visualization group', () => {
    ...
    expect(mod.NAV_GROUPS[0].items.map((i) => i.id).sort()).toEqual(['colony', 'economy', 'graph'])
  })
})
