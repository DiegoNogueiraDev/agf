...
Duas leituras que o número exige, e que os comandos entregam sem que você precise confiar em nós:

**O custo do próprio `agf` tende a zero, e isso é arquitetura, não instrumentação quebrada.** No modo delegate — o padrão quando nenhum provider está conectado — quem paga o LLM é o agente externo que conduz (Claude Code, Copilot, Codex…); o `agf` responde por CLI e não faz chamada. Um `llm_call_ledger` quase vazio é o resultado esperado, não um contador desligado. Confira no seu projeto com `agf metrics`.

**`agf savings` soma dois tipos de linha, e diz qual é qual.** Parte é _medida_ — o corpo renderizado foi contado. Parte é _estimada_ contra uma constante escolhida (`baselineMethod: structural`), porque em modo delegate a economia é um contrafactual: tokens que um agente externo **não** gastou porque o CLI respondeu antes. O envelope traz a quebra por método justamente para você não somar as duas metades sem perceber — rótulo único sobre ledger misto é a mesma falha que rótulo nenhum. Rode `agf savings` e leia `baselineMethods` antes de citar o total.
