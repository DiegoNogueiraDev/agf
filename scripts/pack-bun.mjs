...
import { shouldEmbedIcon, embedWindowsIcon } from './win-icon.mjs'
...
case "$(uname -s)-$(uname -m)" in
  ...
  *) echo "Unsupported OS/arch: $(uname -s)-$(uname -m). On Windows run install-bun.ps1." >&2; exit 1 ;;
esac
SRC="$HERE/$BIN"
...
const INSTALL_PS1 = `# Self-contained agf installer (Windows). Embedded Bun runtime — no Node/npm.
$ErrorActionPreference = "Stop"
$here = Split-Path -Parent $MyInvocation.MyCommand.Path
$src = Join-Path $here "agf-windows-x64.exe"
if (-not (Test-Path $src)) { throw "Missing $src" }
$dest = Join-Path $env:LOCALAPPDATA "Programs\\agf"
New-Item -ItemType Directory -Force -Path $dest | Out-Null
Copy-Item $src (Join-Path $dest "agf.exe") -Force
Write-Host "✓ installed agf → $dest\\agf.exe"
& (Join-Path $dest "agf.exe") --version
...
async function main() {
  ...
  for (const t of targets) {
    ...
    // Embed the ant icon into the Windows PE BEFORE hashing (like the darwin sign,
    // the checksum must cover the final bytes). Best-effort: warns if rcedit absent.
    if (shouldEmbedIcon(t)) {
      const icon = embedWindowsIcon(file, join(ROOT, 'assets', 'agf.ico'), {
        run: (cmd, args) => execFileSync(cmd, args, { stdio: 'inherit' }),
      })
      if (!icon.applied)
        console.warn(`⚠ ${t.out}: icon embed skipped (${icon.reason}) — exe ships without the ant icon`)
    }
  ...
  writeFileSync(join(OUT, 'install-bun.ps1'), INSTALL_PS1)
  writeFileSync(
    join(OUT, 'BUILDINFO'),
